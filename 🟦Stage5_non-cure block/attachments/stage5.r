  ...
  print(
    ggplot(auc_plot_source, aes(x = horizon_year, y = metric_value, color = model_family_plot_label)) +
      ...
      labs(
        ...
        subtitle = "Shown only where the binary IPCW horizon outcome is estimable; this is distinct from the Stage 1 interpretation tier.",
        x = "Prediction horizon (years)",
        y = "AUC",
        color = "Model family"
      ) +
  ...
  print(
    ggplot(calibration_plot_source, aes(x = mean_predicted_risk, y = observed_risk_ipcw, color = model_family_plot_label)) +
      ...
      labs(
        ...
        subtitle = "Selected snapshot panels may include primary-supported, secondary, sensitivity, or projection horizons depending on dataset.",
...
stage5_results_bundle <- list(
  ...
  support_gating_policy = c(
    ...
    "late_horizon_auc_brier_and_decision_curve_retained_when_binary_outcome_support_exists_even_if_known_count_is_small",
...
export_manifest <- tibble(
  ...
  description = c(
    "Model-level fit registry for all Stage 5 non-cure models, including unified anchor-horizon-risk time-range score metadata",
    ...
    "Grouped calibration plot source-of-truth table with horizon support-status flags and unsupported bins suppressed to NA for observed risk",
    "Model-by-horizon calibration fit diagnostics including support, projection-status, convergence, boundary, failure-reason, and suppression flags",
    "Automated Stage 5 QC summary for code-output consistency checks, including support-gated observed IPCW quantities and unified anchor-risk score enforcement",
