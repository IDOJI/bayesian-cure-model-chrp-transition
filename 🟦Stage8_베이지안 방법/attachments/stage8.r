...
# 1) Existing Stage 8 exports are reused as aggressively as possible.
#    If the exported Stage 8 CSV tables are already complete and internally
#    consistent, the script skips model refitting even when model-level RDS
#    files are absent.
# 2) Threshold-based classification rows that are not estimable are preserved
#    as rows, but all classification metrics are suppressed to NA so they are
#    not accidentally interpreted downstream.
# 3) The diagnostic PDF is preserved when all requested models are reused and
#    a usable PDF already exists.

# 🔴 Configure: Stage-8 paths, controls, and reporting thresholds ===============================
merged_data_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧0.생존 데이터 처리와 요약/🟦2.데이터3 처리/attachments/MERGED_dataset3_pnu_snu.csv'
export_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage8_베이지안 방법/attachments'
...
stage6_export_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage6/attachments'
...
# FALSE here means: if the exported Stage 8 CSV tables are already complete and consistent,
# the script may reuse them without requiring model-specific RDS files.
...
combine_unique_strings <- function(..., sep = "; ") {
  vals <- unlist(list(...), recursive = TRUE, use.names = FALSE)
  if (length(vals) == 0L) {
...
# 🔴 Define: schema-preserving helpers ===============================
empty_typed_vector <- function(type, n = 0L) {
  switch(
    type,
    ...
    logical = rep(NA, n),
    stop(sprintf("Unsupported schema type: %s", type), call. = FALSE)
  )
}

make_empty_schema_table <- function(schema) {
  tibble::as_tibble(stats::setNames(lapply(schema, empty_typed_vector), names(schema)))
}

coerce_schema_column <- function(x, type) {
  if (type == "character") {
    return(as.character(x))
  }
  if (type == "integer") {
    return(as.integer(safe_numeric(x)))
  }
  if (type == "double") {
...
ensure_stage8_table_schema <- function(df, schema) {
  out <- if (is.null(df) || !inherits(df, "data.frame")) {
    make_empty_schema_table(schema)
  } else {
    tibble::as_tibble(simplify_scalar_list_cols(df))
  }
  
  if (nrow(out) == 0L && ncol(out) == 0L) {
    out <- make_empty_schema_table(schema)
  }
  
  for (nm in names(schema)) {
    if (!(nm %in% names(out))) {
      out[[nm]] <- empty_typed_vector(schema[[nm]], nrow(out))
    }
    out[[nm]] <- coerce_schema_column(out[[nm]], schema[[nm]])
  }
  
  out %>%
    select(any_of(names(schema)), everything())
}

stage8_schema_coefficient_summary <- c(
  dataset = "character",
  model_id = "character",
  parameter = "character",
  mean = "double",
...
stage8_schema_posterior_subject_profile <- c(
  dataset = "character",
  model_id = "character",
  ...
  site = "character",
  sex_num = "integer",
  age_exact_entry = "double",
  age_s = "double",
  cure_prob_mean = "double",
  cure_prob_q025 = "double",
  cure_prob_q50 = "double",
  cure_prob_q975 = "double",
  susceptible_prob_mean = "double",
  susceptible_prob_q025 = "double",
  susceptible_prob_q50 = "double",
  susceptible_prob_q975 = "double",
  median_susc_time_mean = "double",
  median_susc_time_q025 = "double",
  median_susc_time_q50 = "double",
  median_susc_time_q975 = "double"
)

stage8_schema_posterior_subject_yearly <- c(
  dataset = "character",
  model_id = "character",
  horizon_year = "integer",
  horizon_support_label = "character",
...
stage8_schema_posterior_cohort_yearly <- c(
  dataset = "character",
  model_id = "character",
  latency_family = "character",
  formula_anchor = "character",
  horizon_year = "integer",
  horizon_support_label = "character",
  meanRisk_Bayes_mean = "double",
  meanRisk_Bayes_q025 = "double",
  meanRisk_Bayes_q50 = "double",
  meanRisk_Bayes_q975 = "double",
  meanSurvival_mean = "double",
  meanSurvival_q025 = "double",
  meanSurvival_q50 = "double",
  meanSurvival_q975 = "double",
  meanSusceptibleSurvival_mean = "double",
  meanSusceptibleSurvival_q025 = "double",
  meanSusceptibleSurvival_q50 = "double",
  meanSusceptibleSurvival_q975 = "double",
  meanHazard_mean = "double",
  meanHazard_q025 = "double",
  meanHazard_q50 = "double",
  meanHazard_q975 = "double",
  cohort_mean_cure_fraction_mean = "double",
  cohort_mean_cure_fraction_q025 = "double",
  cohort_mean_cure_fraction_q50 = "double",
  cohort_mean_cure_fraction_q975 = "double",
  support_priority = "integer",
  cohort_reporting_status = "character",
  cohort_reporting_note = "character",
  admissible_flag = "logical",
  admissibility_reasons = "character",
  screening_flag = "character",
  screening_detail = "character",
  fit_reused_flag = "logical"
)

stage8_schema_posterior_classification <- c(
  dataset = "character",
  model_id = "character",
  latency_family = "character",
  formula_anchor = "character",
  ...
  horizon_support_label = "character",
  threshold = "double",
  pos_rate_mean = "double",
  pos_rate_q025 = "double",
  pos_rate_q50 = "double",
  pos_rate_q975 = "double",
  FPR_mean = "double",
  FPR_q025 = "double",
  FPR_q50 = "double",
  FPR_q975 = "double",
  false_positive_burden_mean = "double",
  false_positive_burden_q025 = "double",
  false_positive_burden_q50 = "double",
  false_positive_burden_q975 = "double",
  FP100_mean = "double",
  FP100_q025 = "double",
  FP100_q50 = "double",
  FP100_q975 = "double",
  ...
  NB_q50 = "double",
  NB_q975 = "double",
  observed_km_risk = "double",
  support_priority = "integer",
  denom_case = "double",
  denom_control = "double",
  classification_estimable_flag = "logical",
  classification_reporting_status = "character",
  classification_reporting_note = "character",
  admissible_flag = "logical",
  admissibility_reasons = "character",
  screening_flag = "character",
  screening_detail = "character",
...
stage8_schema_diagnostics_parameter_level <- c(
  dataset = "character",
  model_id = "character",
  parameter = "character",
  mean = "double",
...
stage8_schema_ppc_summary <- c(
  dataset = "character",
  model_id = "character",
  horizon_year = "integer",
  horizon_support_label = "character",
  observed_km_risk = "double",
  posterior_mean_risk = "double",
  posterior_q025_risk = "double",
  posterior_q975_risk = "double",
  absolute_difference = "double",
  gross_contradiction_flag = "logical",
  support_priority = "integer",
  cohort_reporting_status = "character",
  cohort_reporting_note = "character",
  admissible_flag = "logical",
...
classification_metric_columns <- c(
  "pos_rate_mean", "pos_rate_q025", "pos_rate_q50", "pos_rate_q975",
  "FPR_mean", "FPR_q025", "FPR_q50", "FPR_q975",
  "false_positive_burden_mean", "false_positive_burden_q025", "false_positive_burden_q50", "false_positive_burden_q975",
  "FP100_mean", "FP100_q025", "FP100_q50", "FP100_q975",
  "PPV_mean", "PPV_q025", "PPV_q50", "PPV_q975",
  "FDP_mean", "FDP_q025", "FDP_q50", "FDP_q975",
  "TPR_mean", "TPR_q025", "TPR_q50", "TPR_q975",
  "NB_mean", "NB_q025", "NB_q50", "NB_q975"
)

suppress_non_estimable_classification_metrics <- function(df) {
  out <- if (is.null(df) || !inherits(df, "data.frame")) tibble() else tibble::as_tibble(df)
  if (nrow(out) == 0L) {
    return(out)
  }
  if (!("classification_estimable_flag" %in% names(out))) {
    return(out)
  }
  
  suppress_idx <- !dplyr::coalesce(as.logical(out$classification_estimable_flag), FALSE)
  if ("classification_reporting_status" %in% names(out)) {
    suppress_idx <- suppress_idx | (as.character(out$classification_reporting_status) == "not_estimable")
  }
  
  metric_cols <- intersect(classification_metric_columns, names(out))
  for (nm in metric_cols) {
    out[[nm]][suppress_idx] <- NA_real_
  }
  out
}

ensure_prior_sensitivity_columns <- function(df) {
  out <- if (is.null(df) || !inherits(df, "data.frame")) tibble() else tibble::as_tibble(df)
  ...
  required_double <- c(
    ...
    "q025__wider_latency",
    "q975__wider_latency",
    "delta_mean_wider_minus_main"
  )
  required_logical <- c(
    "prior_degenerate_flag__main",
    "prior_tail_warning_flag__main",
    "prior_degenerate_flag__wider_latency",
    "prior_tail_warning_flag__wider_latency",
    "prior_tail_warning_flag_any"
  )
  required_character <- c("dataset", "model_id", "metric")
  required_integer <- c("horizon_year")
  
  for (nm in required_character) {
    if (!(nm %in% names(out))) out[[nm]] <- empty_typed_vector("character", nrow(out))
    out[[nm]] <- as.character(out[[nm]])
  }
  for (nm in required_integer) {
    if (!(nm %in% names(out))) out[[nm]] <- empty_typed_vector("integer", nrow(out))
    out[[nm]] <- as.integer(safe_numeric(out[[nm]]))
  }
  for (nm in required_double) {
    if (!(nm %in% names(out))) out[[nm]] <- empty_typed_vector("double", nrow(out))
    out[[nm]] <- safe_numeric(out[[nm]])
  }
  for (nm in required_logical) {
    if (!(nm %in% names(out))) out[[nm]] <- empty_typed_vector("logical", nrow(out))
    out[[nm]] <- as.logical(out[[nm]])
  }
  
  out %>%
    select(
      any_of(c(required_character, required_integer, required_double, required_logical)),
      everything()
    )
}

empty_prior_warning_registry <- function() {
  tibble(
    dataset = character(),
    model_id = character(),
    prior_tail_warning_flag_main = logical(),
    prior_tail_warning_detail_main = character(),
    prior_tail_warning_flag_wider_latency = logical(),
    prior_tail_warning_detail_wider_latency = character(),
    prior_tail_warning_flag_any = logical(),
    prior_tail_warning_detail_any = character()
  )
}
...
read_screening_flags <- function(path) {
  ...
  if (("final_decision_flag" %in% names(df)) && any(c("dataset_key", "source_dataset") %in% names(df))) {
    ...
    out_stage6 <- bind_rows(lapply(seq_len(nrow(df)), function(i) {
      one <- df[i, , drop = FALSE]
      dataset_value <- if (!is.null(dataset_source_col)) one[[dataset_source_col]][[1]] else NA_character_
      ...
      tibble(
        ...
        flag = as.character(one$final_decision_flag[[1]]),
        method = paste0(
          "stage6_",
          if (!is.null(analysis_variant_col)) as.character(one[[analysis_variant_col]][[1]]) else "screening"
        )
      )
    })) %>%
      filter(dataset %in% c("PNU", "SNU", "merged"))
    
    if (nrow(out_stage6) == 0) {
      return(empty_screening_template())
    }
    ...
    return(
      out_stage6 %>%
        ...
        summarise(
          screening_flag = paste(unique(na.omit(flag)), collapse = "|"),
          screening_detail = paste(unique(paste0(method, ":", flag)), collapse = "; "),
          .groups = "drop"
        )
    )
  }
  ...
  flag_col <- first_existing_name(df, c("final_decision_flag", "decision_flag", "screening_flag"))
  method_col <- first_existing_name(df, c("method", "screening_method", "analysis_variant"))
  ...
  out <- df %>%
    transmute(
      ...
      flag = as.character(.data[[flag_col]]),
      method = if (!is.null(method_col)) as.character(.data[[method_col]]) else "screening"
    ) %>%
    filter(dataset %in% c("PNU", "SNU", "merged")) %>%
    group_by(dataset, formula_anchor) %>%
    summarise(
  ...
  exact_lookup <- model_base %>%
    left_join(
      screening_flags %>%
        ...
        distinct(dataset, formula_anchor, screening_flag, screening_detail),
      by = c("dataset", "formula_anchor")
    ) %>%
    select(model_id, screening_flag, screening_detail)
  
  all_lookup <- model_base %>%
    left_join(
      screening_flags %>%
        filter(formula_anchor == "ALL") %>%
        distinct(dataset, screening_flag, screening_detail) %>%
        rename(
          screening_flag_all = screening_flag,
          screening_detail_all = screening_detail
        ),
      by = "dataset"
    ) %>%
    select(model_id, screening_flag_all, screening_detail_all)
  
  exact_lookup %>%
    left_join(all_lookup, by = "model_id") %>%
    transmute(
      model_id = model_id,
      screening_flag = dplyr::coalesce(screening_flag, screening_flag_all),
      screening_detail = dplyr::coalesce(screening_detail, screening_detail_all)
    )
}
...
rebuild_delta_vs_nocure <- function(posterior_cohort_yearly, posterior_classification, nocure_cohort_long = NULL, nocure_class_long = NULL) {
  ...
  if (!is.null(nocure_class_long) && nrow(nocure_class_long) > 0 && nrow(posterior_classification) > 0) {
    ...
    class_long <- bind_rows(lapply(seq_len(nrow(metric_map)), function(i) {
      ...
      posterior_classification %>%
        filter(dplyr::coalesce(.data$classification_estimable_flag, FALSE)) %>%
        transmute(
          dataset = dataset,
          model_id = model_id,
...
if (!isTRUE(require_existing_rds_to_skip_fit)) {
  message("Stage 8 reuse rule: complete existing CSV exports may be reused without model-specific RDS files.")
}
if (n_models_to_fit == 0L && isTRUE(reuse_existing_stage8_outputs)) {
  message("All requested model fits are already available in `export_path`; Stage 8 will rebuild final exports without refitting.")
}
...
model_registry <- bind_rows_safe(main_registry_rows) %>%
  select(-any_of(c(
    ...
    "prior_tail_warning_flag_main",
    "prior_tail_warning_detail_main",
    "prior_tail_warning_flag_wider_latency",
    "prior_tail_warning_detail_wider_latency",
    "prior_tail_warning_flag_any",
    "prior_tail_warning_detail_any"
  ))) %>%
  mutate(
    rds_path = ifelse(
  ...
  left_join(
    screening_lookup %>%
      rename(
        screening_flag_import = screening_flag,
        screening_detail_import = screening_detail
      ),
    by = "model_id"
  ) %>%
  mutate(
    screening_flag = dplyr::coalesce(screening_flag_import, screening_flag),
    screening_detail = dplyr::coalesce(screening_detail_import, screening_detail)
  ) %>%
  select(-any_of(c("screening_flag_import", "screening_detail_import"))) %>%
  arrange(factor(model_id, levels = model_order))

coefficient_summary <- ensure_stage8_table_schema(
  bind_rows_safe(coef_rows),
  stage8_schema_coefficient_summary
) %>%
  arrange(factor(model_id, levels = model_order), parameter)

posterior_subject_profile <- ensure_stage8_table_schema(
  bind_rows_safe(subject_profile_rows),
  stage8_schema_posterior_subject_profile
) %>%
  arrange(factor(model_id, levels = model_order), unique_person_id)

posterior_subject_yearly <- ensure_stage8_table_schema(
  bind_rows_safe(subject_yearly_rows),
  stage8_schema_posterior_subject_yearly
) %>%
  arrange(factor(model_id, levels = model_order), horizon_year, unique_person_id)

posterior_cohort_yearly <- ensure_stage8_table_schema(
  bind_rows_safe(cohort_yearly_rows),
  stage8_schema_posterior_cohort_yearly
) %>%
  arrange(factor(model_id, levels = model_order), horizon_year)

posterior_classification <- ensure_stage8_table_schema(
  bind_rows_safe(class_rows),
  stage8_schema_posterior_classification
) %>%
  arrange(factor(model_id, levels = model_order), horizon_year, threshold)

diagnostics_parameter_level <- ensure_stage8_table_schema(
  bind_rows_safe(diagnostic_rows),
  stage8_schema_diagnostics_parameter_level
) %>%
  arrange(factor(model_id, levels = model_order), parameter)

ppc_summary <- ensure_stage8_table_schema(
  bind_rows_safe(ppc_rows_all),
  stage8_schema_ppc_summary
) %>%
  arrange(factor(model_id, levels = model_order), horizon_year)
...
prior_sensitivity_summary <- prior_predictive_summary %>%
  ...
  ensure_prior_sensitivity_columns() %>%
  mutate(
...
prior_warning_by_set <- prior_predictive_summary %>%
  group_by(dataset, model_id, prior_set) %>%
  summarise(
    prior_tail_warning_flag = any(dplyr::coalesce(.data$prior_tail_warning_flag, FALSE)),
    prior_tail_warning_detail = combine_unique_strings(
      .data$prior_tail_warning_detail[dplyr::coalesce(.data$prior_tail_warning_flag, FALSE)]
    ),
    .groups = "drop"
  )

if (nrow(prior_warning_by_set) == 0L) {
  prior_warning_by_model <- empty_prior_warning_registry()
} else {
  prior_warning_wide <- prior_warning_by_set %>%
    tidyr::pivot_wider(
      names_from = prior_set,
      values_from = c(prior_tail_warning_flag, prior_tail_warning_detail),
      names_sep = "__"
    )
  
  if (!("prior_tail_warning_flag__main" %in% names(prior_warning_wide))) {
    prior_warning_wide$prior_tail_warning_flag__main <- FALSE
  }
  if (!("prior_tail_warning_detail__main" %in% names(prior_warning_wide))) {
    prior_warning_wide$prior_tail_warning_detail__main <- NA_character_
  }
  if (!("prior_tail_warning_flag__wider_latency" %in% names(prior_warning_wide))) {
    prior_warning_wide$prior_tail_warning_flag__wider_latency <- FALSE
  }
  if (!("prior_tail_warning_detail__wider_latency" %in% names(prior_warning_wide))) {
    prior_warning_wide$prior_tail_warning_detail__wider_latency <- NA_character_
  }
  
  prior_warning_by_model <- prior_warning_wide %>%
    transmute(
      dataset = dataset,
      model_id = model_id,
      prior_tail_warning_flag_main = dplyr::coalesce(prior_tail_warning_flag__main, FALSE),
      prior_tail_warning_detail_main = as.character(prior_tail_warning_detail__main),
      prior_tail_warning_flag_wider_latency = dplyr::coalesce(prior_tail_warning_flag__wider_latency, FALSE),
      prior_tail_warning_detail_wider_latency = as.character(prior_tail_warning_detail__wider_latency)
    )
  
  prior_warning_by_model$prior_tail_warning_flag_any <- with(
    prior_warning_by_model,
    prior_tail_warning_flag_main | prior_tail_warning_flag_wider_latency
  )
  prior_warning_by_model$prior_tail_warning_detail_any <- vapply(
    seq_len(nrow(prior_warning_by_model)),
    function(i) combine_unique_strings(
      prior_warning_by_model$prior_tail_warning_detail_main[[i]],
      prior_warning_by_model$prior_tail_warning_detail_wider_latency[[i]]
    ),
    character(1)
  )
}
...
model_registry <- model_registry %>%
  ...
  mutate(
    prior_tail_warning_flag_main = dplyr::coalesce(prior_tail_warning_flag_main, FALSE),
    prior_tail_warning_flag_wider_latency = dplyr::coalesce(prior_tail_warning_flag_wider_latency, FALSE),
    prior_tail_warning_flag_any = dplyr::coalesce(prior_tail_warning_flag_any, FALSE),
    prior_tail_warning_flag = prior_tail_warning_flag_any,
    prior_tail_warning_detail = prior_tail_warning_detail_any
  ) %>%
  arrange(factor(model_id, levels = model_order))
...
posterior_classification <- posterior_classification %>%
  ...
  suppress_non_estimable_classification_metrics() %>%
  arrange(factor(model_id, levels = model_order), horizon_year, threshold)
...
output_audit <- make_stage8_output_audit(
