...
## 🟠 Define: Stage 1 registry loaders ===============================
...
## 🟠 Define: Stage 1-based column patchers ===============================
...
## 🟠 Define: hazard and fit-contrast builders ===============================
standardize_fit_metric_columns <- function(df) {
  if (nrow(df) == 0L) {
    return(df)
  }

  std_logLik_vec <- first_existing_value(df, c("logLik", "loglik", "fit_logLik", "model_logLik", "maximized_logLik", "max_logLik", "log_likelihood", "ll"))
  std_n_parameters_vec <- first_existing_value(df, c("n_parameters", "parameter_count", "df_model", "effective_parameter_n"))
  sample_size_vec <- first_existing_value(df, c("n", "sample_size", "n_subject", "n_subjects", "n_obs", "n_observations", "n_total", "n_rows", "analysis_n", "N"))
  std_AIC_existing <- first_existing_value(df, c("AIC", "fit_AIC", "model_AIC", "aic"))
  std_BIC_existing <- first_existing_value(df, c("BIC", "fit_BIC", "model_BIC", "bic"))
  std_AIC_calc <- ifelse(is.finite(std_logLik_vec) & is.finite(std_n_parameters_vec), -2 * std_logLik_vec + 2 * std_n_parameters_vec, NA_real_)
  std_BIC_calc <- ifelse(
    is.finite(std_logLik_vec) & is.finite(std_n_parameters_vec) & is.finite(sample_size_vec),
    -2 * std_logLik_vec + log(pmax(sample_size_vec, 1)) * std_n_parameters_vec,
    NA_real_
  )

  df %>%
    mutate(
      std_logLik = std_logLik_vec,
      std_AIC = dplyr::coalesce(std_AIC_existing, std_AIC_calc),
      std_BIC = dplyr::coalesce(std_BIC_existing, std_BIC_calc),
      std_n_parameters = std_n_parameters_vec,
...
stage7_detect_cure_model_flag <- function(df) {
  role_txt <- tolower(first_existing_character(df, c("model_block", "model_class", "model_id"), default = ""))
...
stage7_apply_convergence_guard <- function(fit_registry_tbl) {
  fit_registry_tbl <- tibble::as_tibble(fit_registry_tbl)
  if (nrow(fit_registry_tbl) == 0L) {
    return(fit_registry_tbl)
  }

  fit_registry_tbl <- ensure_columns_exist(
    fit_registry_tbl,
    list(
      has_converged_solution = NA,
      overall_status = NA_character_,
      warning_message = NA_character_,
      non_admissible_fit_flag = NA
    )
  )

  is_cure_vec <- stage7_detect_cure_model_flag(fit_registry_tbl)
  has_converged_vec <- suppressWarnings(as.logical(fit_registry_tbl$has_converged_solution))
  warning_vec <- as.character(fit_registry_tbl$warning_message)
  warning_vec[is.na(warning_vec)] <- ""
  non_admissible_vec <- is_cure_vec & !dplyr::coalesce(has_converged_vec, FALSE)
  guard_note <- "non-converged fit excluded from primary summaries"

  fit_registry_tbl %>%
    mutate(
      non_admissible_fit_flag = dplyr::coalesce(as.logical(non_admissible_fit_flag), FALSE) | non_admissible_vec,
      overall_status = dplyr::case_when(
        non_admissible_fit_flag ~ "ok_but_nonconverged_exclude_from_primary",
        TRUE ~ as.character(overall_status)
      ),
      warning_message = dplyr::case_when(
        non_admissible_fit_flag & stringr::str_detect(warning_vec, stringr::fixed(guard_note)) ~ warning_vec,
        non_admissible_fit_flag & nzchar(warning_vec) ~ paste0(warning_vec, " | ", guard_note),
        non_admissible_fit_flag ~ guard_note,
        TRUE ~ as.character(warning_message)
      )
    )
}

stage7_primary_fit_keys <- function(fit_registry_tbl) {
  fit_registry_tbl %>%
    tibble::as_tibble() %>%
    ...
    filter(overall_status != "ok_but_nonconverged_exclude_from_primary") %>%
    distinct(dataset, formula_variant, site_branch, interaction_branch, model_id)
}

stage7_filter_primary_export_tbl <- function(tbl, primary_fit_keys) {
  tbl <- tibble::as_tibble(tbl)
  if (nrow(tbl) == 0L) {
    return(tbl)
  }
  if (nrow(primary_fit_keys) == 0L) {
    return(tbl[0, ])
  }
  ...
  tbl %>%
    ...
    semi_join(primary_fit_keys, by = c("dataset", "formula_variant", "site_branch", "interaction_branch", "model_id"))
}

stage7_assert_postpatch_qc <- function(fit_registry_tbl, family_contrast_tbl, cure_decomp_tbl) {
  fit_registry_tbl <- tibble::as_tibble(fit_registry_tbl)
  family_contrast_tbl <- tibble::as_tibble(family_contrast_tbl)
  cure_decomp_tbl <- tibble::as_tibble(cure_decomp_tbl)

  if (nrow(fit_registry_tbl) > 0L) {
    is_cure_vec <- stage7_detect_cure_model_flag(fit_registry_tbl)
    has_converged_vec <- suppressWarnings(as.logical(fit_registry_tbl$has_converged_solution))
    overall_status_vec <- as.character(fit_registry_tbl$overall_status)
    bad_nonconv <- fit_registry_tbl[is_cure_vec & !dplyr::coalesce(has_converged_vec, FALSE) & overall_status_vec == "ok", , drop = FALSE]
    if (nrow(bad_nonconv) > 0L) {
      stop("Post-patch QC failed: non-converged cure fits still labeled overall_status == 'ok'.", call. = FALSE)
    }
  }

  if (nrow(family_contrast_tbl) > 0L && all(is.na(family_contrast_tbl$logLik_cure))) {
    stop("Post-patch QC failed: family contrast has no cure log-likelihood values.", call. = FALSE)
  }

  if (nrow(cure_decomp_tbl) > 0L && "uncured_mean_support_flag" %in% names(cure_decomp_tbl)) {
    support_flag_vec <- dplyr::coalesce(as.character(cure_decomp_tbl$uncured_mean_support_flag), "")
    if (any(stringr::str_detect(support_flag_vec, "grid_proxy"), na.rm = TRUE)) {
      stop("Post-patch QC failed: proxy MSTu is still being exported as a dedicated support flag.", call. = FALSE)
    }
  }
...
build_family_matched_fit_contrast <- function(fit_registry_tbl, stage5_fit_tbl = tibble()) {
  ...
  fit_registry_tbl <- fit_registry_tbl %>%
    tibble::as_tibble() %>%
    standardize_fit_metric_columns()

  cure_df <- fit_registry_tbl %>%
    filter(stage7_detect_cure_model_flag(.)) %>%
    mutate(
  ...
  out <- cure_df %>%
    left_join(
      noncure_df,
      by = c("dataset_key", "formula_variant", "matched_noncure_model_id")
    ) %>%
    mutate(
      logLik_cure = std_logLik,
    ...
    transmute(
      ...
      logLik_cure = as.numeric(logLik_cure),
      delta_logLik_cure_minus_noncure = as.numeric(delta_logLik_cure_minus_noncure),
  ...
  if (nrow(out) > 0L && all(is.na(out$logLik_cure))) {
    stop("stage7_family_matched_fit_contrast: all logLik_cure values are NA after join.", call. = FALSE)
  }
...
build_cure_supporting_decomposition <- function(fit_registry_tbl, risk_summary_tbl, subject_predictions_tbl = tibble()) {
  output_schema <- tibble(
    dataset = character(),
    dataset_key = character(),
    formula_variant = character(),
  ...
  fit_registry_tbl <- tibble::as_tibble(fit_registry_tbl)
  risk_summary_tbl <- tibble::as_tibble(risk_summary_tbl)
  subject_predictions_tbl <- tibble::as_tibble(subject_predictions_tbl)
  ...
  cure_model_tbl <- fit_registry_tbl %>%
    filter(stage7_detect_cure_model_flag(.))

  model_level <- cure_model_tbl %>%
    mutate(
      cure_fraction = first_existing_value(cure_model_tbl, c("cure_fraction", "estimated_cure_fraction", "mean_cure_fraction", "pi_hat", "cure_rate", "cure_prob", "prob_cure")),
      susceptible_fraction = first_existing_value(cure_model_tbl, c("susceptible_fraction", "estimated_susceptible_fraction", "mean_susceptible_fraction")),
      MSTu = first_existing_value(cure_model_tbl, c("MSTu", "mstu", "uncured_mean_survival", "mean_survival_uncured", "mst_uncured"))
    ) %>%
    mutate(
  ...
  combined <- if (nrow(annual_tbl) > 0L) {
    ...
    model_level %>%
      mutate(horizon_year = NA_integer_, uncured_survival = NA_real_, uncured_risk = NA_real_, cure_fraction_h = NA_real_)
  }

  reported_MSTu_vec <- suppressWarnings(as.numeric(combined$MSTu))

  out <- combined %>%
    mutate(
      cure_fraction = dplyr::coalesce(cure_fraction_h, cure_fraction),
      susceptible_fraction = dplyr::coalesce(susceptible_fraction, ifelse(!is.na(cure_fraction), 1 - cure_fraction, NA_real_)),
      MSTu = dplyr::case_when(
        !is.na(reported_MSTu_vec) & dplyr::coalesce(stage6__followup_not_contradicted_flag, FALSE) ~ reported_MSTu_vec,
        TRUE ~ NA_real_
      ),
      uncured_mean_support_flag = dplyr::case_when(
        !is.na(reported_MSTu_vec) & dplyr::coalesce(stage6__followup_not_contradicted_flag, FALSE) ~ "reported_from_fit_registry",
        (!is.na(uncured_survival) | !is.na(uncured_risk)) ~ "annual_grid_only_no_supported_MSTu",
...
## 🟠 Define: visual summary plot factories ===============================
...
## 🟠 Define: core-output reuse checks and root resolution ===============================
...
## 🟠 Define: Stage 7 formula plans and model catalog ===============================
...
## 🟠 Define: core-fit numerical helpers ===============================
...
stage7_parametric_mean_survival <- function(lp, family, aux_log = NA_real_) {
  lp <- as.numeric(lp)
  if (family == "exp") {
    exp(lp)
  } else if (family == "weibull") {
    sigma <- exp(aux_log)
    exp(lp) * gamma(1 + sigma)
  } else if (family == "lnorm") {
    sigma <- exp(aux_log)
    exp(lp + 0.5 * sigma^2)
  } else if (family == "llogis") {
    sigma <- exp(aux_log)
    out <- rep(NA_real_, length(lp))
    keep <- sigma < 1
    out[keep] <- exp(lp[keep]) * (pi * sigma) / sin(pi * sigma)
    out
  } else {
    rep(NA_real_, length(lp))
  }
}
...
## 🟠 Define: no-cure, benchmark, and cure fitters ===============================
...
stage7_predict_smcure <- function(fit_result, df, incidence_rhs, latency_rhs, horizons) {
  ...
  if (model_type == "ph") {
    ...
    if (is.null(dim(susceptible_surv_mat))) susceptible_surv_mat <- matrix(susceptible_surv_mat, nrow = nrow(df), ncol = length(horizons))
  } else {
    lat_coef_names <- fit$betanm %||% names(fit$beta)
    mm_lat_raw <- stage7_model_matrix(df, latency_rhs, drop_intercept = FALSE)
    X <- stage7_align_mm_to_coef(mm_lat_raw, lat_coef_names)
...
## 🟠 Define: stage7 core summaries from subject predictions ===============================
...
## 🟠 Define: stage7 core fitting driver ===============================
...
## 🟠 Transform: harmonize frequentist cure exports ===============================
...
fit_registry_tbl <- stage7_apply_convergence_guard(fit_registry_tbl)
primary_fit_keys <- stage7_primary_fit_keys(fit_registry_tbl)
subject_predictions_tbl <- stage7_filter_primary_export_tbl(subject_predictions_tbl, primary_fit_keys)
risk_summary_tbl <- stage7_filter_primary_export_tbl(risk_summary_tbl, primary_fit_keys)
delta_risk_tbl <- stage7_filter_primary_export_tbl(delta_risk_tbl, primary_fit_keys)
threshold_metrics_tbl <- stage7_filter_primary_export_tbl(threshold_metrics_tbl, primary_fit_keys)

# 🔴 Rebuild: IPCW registry and new Stage 7 support tables ===============================
## 🟠 Rebuild: Stage 1-dependent derived outputs ===============================
...
if (should_rebuild_output(cure_decomp_file_target, force_rebuild = refresh_new_spec_tables_even_when_outputs_exist, required_cols = c("dataset", "formula_variant", "model_id", "susceptible_fraction", "risk_scale", "uncured_mean_support_flag"))) {
  cure_supporting_decomposition_tbl <- build_cure_supporting_decomposition(fit_registry_tbl, risk_summary_tbl, subject_predictions_tbl)
} else {
...
## 🟠 Regenerate: plot data and image exports ===============================
...
# 🔴 Export: refreshed Stage 7 tables ===============================
...
stage7_assert_postpatch_qc(fit_registry_tbl, family_matched_fit_contrast_tbl, cure_supporting_decomposition_tbl)
...
## 🟠 Assemble: one manifest covering refreshed outputs ===============================
...
stage7_v9_extract_sample_size <- function(df, default = NA_real_) {
  stage7_v9_first_nonmissing_num(
    df,
    c('n', 'sample_size', 'n_subject', 'n_subjects', 'n_obs', 'n_observations', 'n_total', 'n_rows', 'analysis_n', 'N'),
    default = default
  )
}
...
stage7_v9_site_interpretation_fallback <- function(dataset, uses_site, existing_value = NA_character_) {
  ...
  use_site_flag <- dplyr::coalesce(as.logical(uses_site), FALSE)
  need <- is.na(out) | out == ''
  out[need & ds == 'merged' & use_site_flag] <- 'structural_context_proxy_not_causal_treatment_effect'
  out[need & !(ds == 'merged' & use_site_flag)] <- 'not_applicable'
  out
}
...
stage7_v9_patch_metadata <- function(df) {
  ...
  out <- df %>%
    mutate(
      ...
      site_term_interpretation = stage7_v9_site_interpretation_fallback(dataset, uses_site, site_term_interpretation)
    )

  out$dataset_key <- stage7_v9_coalesce_dataset_key(out)
  out$incidence_link <- stage7_v9_incidence_link(out)
...
stage7_v9_patch_fit_metrics <- function(fit_registry_tbl, coefficients_tbl = tibble()) {
  ...
  logLik_vec <- stage7_v9_first_nonmissing_num(out, c('logLik', 'loglik', 'fit_logLik', 'model_logLik', 'maximized_logLik', 'max_logLik', 'log_likelihood', 'll'))
  n_parameters_existing <- stage7_v9_first_nonmissing_num(out, c('n_parameters', 'parameter_count', 'df_model', 'effective_parameter_n'))
  n_parameters_vec <- dplyr::coalesce(n_parameters_existing, suppressWarnings(as.numeric(out$n_parameters_from_coeff)))
  sample_size_vec <- stage7_v9_extract_sample_size(out)
  AIC_existing <- stage7_v9_first_nonmissing_num(out, c('AIC', 'fit_AIC', 'model_AIC', 'aic'))
  BIC_existing <- stage7_v9_first_nonmissing_num(out, c('BIC', 'fit_BIC', 'model_BIC', 'bic'))
  AIC_calc <- ifelse(is.finite(logLik_vec) & is.finite(n_parameters_vec), -2 * logLik_vec + 2 * n_parameters_vec, NA_real_)
  BIC_calc <- ifelse(
    is.finite(logLik_vec) & is.finite(n_parameters_vec) & is.finite(sample_size_vec),
    -2 * logLik_vec + log(pmax(sample_size_vec, 1)) * n_parameters_vec,
    NA_real_
  )
  ...
  out <- out %>%
    mutate(
      logLik = logLik_vec,
      n_parameters = n_parameters_vec,
      AIC = dplyr::coalesce(AIC_existing, AIC_calc),
      BIC = dplyr::coalesce(BIC_existing, BIC_calc)
    ) %>%
...
stage7_v9_standardize_fit_metric_columns <- function(df) {
  ...
  std_logLik_vec <- stage7_v9_first_nonmissing_num(df, c('logLik', 'loglik', 'fit_logLik', 'model_logLik', 'maximized_logLik', 'max_logLik', 'log_likelihood', 'll'))
  std_n_parameters_vec <- stage7_v9_first_nonmissing_num(df, c('n_parameters', 'parameter_count', 'df_model', 'effective_parameter_n'))
  sample_size_vec <- stage7_v9_extract_sample_size(df)
  std_AIC_existing <- stage7_v9_first_nonmissing_num(df, c('AIC', 'fit_AIC', 'model_AIC', 'aic'))
  std_BIC_existing <- stage7_v9_first_nonmissing_num(df, c('BIC', 'fit_BIC', 'model_BIC', 'bic'))
  status_vec <- stage7_v9_first_nonmissing_chr(df, c('overall_status', 'fit_status', 'status', 'convergence_status'), default = NA_character_)
  role_source <- tolower(stage7_v9_first_nonmissing_chr(df, c('model_block', 'model_class', 'model_id'), default = ''))
  std_model_role_vec <- dplyr::case_when(
    stringr::str_detect(role_source, 'no[-_ ]?cure') ~ 'no_cure',
    stringr::str_detect(role_source, 'cure') ~ 'cure',
    TRUE ~ NA_character_
  )
  std_AIC_calc <- ifelse(is.finite(std_logLik_vec) & is.finite(std_n_parameters_vec), -2 * std_logLik_vec + 2 * std_n_parameters_vec, NA_real_)
  std_BIC_calc <- ifelse(
  ...
  df %>%
    mutate(
      ...
      std_AIC = dplyr::coalesce(std_AIC_existing, std_AIC_calc),
      std_BIC = dplyr::coalesce(std_BIC_existing, std_BIC_calc),
      std_convergence_status = status_vec,
      std_model_role = std_model_role_vec
    )
}
stage7_v9_build_family_matched_fit_contrast <- function(fit_registry_tbl, stage5_fit_tbl = tibble(), coefficients_tbl = tibble()) {
  output_schema <- tibble(
  ...
  fit_registry_tbl <- fit_registry_tbl %>%
    tibble::as_tibble() %>%
    stage7_v9_patch_fit_metrics(coefficients_tbl = coefficients_tbl) %>%
    stage7_v9_standardize_fit_metric_columns() %>%
    stage7_v9_patch_metadata()
  ...
  fit_registry_tbl <- fit_registry_tbl %>%
    mutate(
      ...
      .is_cure_model = stage7_v9_is_cure_model(.)
    )
...
stage7_v9_build_cure_supporting_decomposition <- function(fit_registry_tbl, risk_summary_tbl, subject_predictions_tbl = tibble()) {
  ...
  fit_registry_aug <- fit_registry_tbl
  fit_registry_aug$.is_cure_model <- stage7_v9_is_cure_model(fit_registry_aug)
  cure_model_level <- fit_registry_aug %>% filter(.is_cure_model)
  if (nrow(cure_model_level) == 0L) return(output_schema)

  cure_fraction_vec <- stage7_v9_first_nonmissing_num(cure_model_level, c('cure_fraction', 'estimated_cure_fraction', 'mean_cure_fraction', 'pi_hat', 'cure_rate', 'cure_prob', 'prob_cure'))
  susceptible_fraction_vec <- stage7_v9_first_nonmissing_num(cure_model_level, c('susceptible_fraction', 'estimated_susceptible_fraction', 'mean_susceptible_fraction'))
  MSTu_vec <- stage7_v9_first_nonmissing_num(cure_model_level, c('MSTu', 'mstu', 'uncured_mean_survival', 'mean_survival_uncured', 'mst_uncured'))

  cure_model_level <- cure_model_level %>%
    mutate(
      cure_fraction = cure_fraction_vec,
      susceptible_fraction = susceptible_fraction_vec,
      MSTu = MSTu_vec,
  ...
  annual_tbl <- tibble()
  if (!is.na(uncured_survival_col) || !is.na(uncured_risk_col) || !is.na(cure_fraction_col)) {
    risk_summary_aug <- risk_summary_tbl
    risk_summary_aug$.is_cure_model <- stage7_v9_is_cure_model(risk_summary_aug)
    annual_tbl <- risk_summary_aug %>%
      filter(.is_cure_model) %>%
      transmute(
        dataset = dataset,
  ...
  if (nrow(annual_tbl) == 0L && nrow(subject_predictions_tbl) > 0L) {
    subject_predictions_aug <- subject_predictions_tbl
    subject_predictions_aug$.is_cure_model <- stage7_v9_is_cure_model(subject_predictions_aug)
    annual_tbl <- subject_predictions_aug %>%
      filter(.is_cure_model) %>%
      group_by(dataset, dataset_key, formula_variant, formula_label, site_branch, interaction_branch, site_placement_label, model_id, model_class, model_family, latency_type, risk_scale, horizon_year) %>%
      summarise(
        uncured_survival = mean(suppressWarnings(as.numeric(predicted_survival_susceptible)), na.rm = TRUE),
  ...
  combined <- if (nrow(annual_tbl) > 0L) {
    ...
    cure_model_level %>% mutate(horizon_year = NA_integer_, uncured_survival = NA_real_, uncured_risk = NA_real_, cure_fraction_h = NA_real_)
  }
  ...
  out <- combined %>%
    mutate(
      ...
      MSTu = dplyr::case_when(
        !is.na(reported_MSTu_vec) &
          dplyr::coalesce(stage6__followup_not_contradicted_flag, FALSE) ~ reported_MSTu_vec,
        TRUE ~ NA_real_
      ),
      uncured_mean_support_flag = dplyr::case_when(
        !is.na(reported_MSTu_vec) &
          dplyr::coalesce(stage6__followup_not_contradicted_flag, FALSE) ~ 'reported_from_fit_registry',
        (!is.na(uncured_survival) | !is.na(uncured_risk)) ~ 'annual_grid_only_no_supported_MSTu',
        TRUE ~ 'not_available_from_stage7_outputs'
      ),
      source_uncured_survival_col = dplyr::coalesce(uncured_survival_col, 'predicted_survival_susceptible'),
...
stage7_v9_fit_registry_tbl <- stage7_apply_convergence_guard(stage7_v9_fit_registry_tbl)
...
stage7_v9_primary_fit_keys <- stage7_primary_fit_keys(stage7_v9_fit_registry_tbl)
stage7_v9_risk_summary_tbl <- stage7_filter_primary_export_tbl(stage7_v9_risk_summary_tbl, stage7_v9_primary_fit_keys)
stage7_v9_delta_risk_tbl <- stage7_filter_primary_export_tbl(stage7_v9_delta_risk_tbl, stage7_v9_primary_fit_keys)
stage7_v9_threshold_metrics_tbl <- stage7_filter_primary_export_tbl(stage7_v9_threshold_metrics_tbl, stage7_v9_primary_fit_keys)
stage7_v9_subject_predictions_tbl <- stage7_filter_primary_export_tbl(stage7_v9_subject_predictions_tbl, stage7_v9_primary_fit_keys)
...
stage7_v9_family_contrast_tbl <- stage7_v9_build_family_matched_fit_contrast(stage7_v9_fit_registry_tbl, stage7_v9_stage5_tbl, stage7_v9_coefficients_tbl)
...
stage7_assert_postpatch_qc(stage7_v9_fit_registry_tbl, stage7_v9_family_contrast_tbl, stage7_v9_cure_decomp_tbl)
