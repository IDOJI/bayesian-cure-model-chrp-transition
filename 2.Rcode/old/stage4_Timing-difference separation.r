...
export_path <- '/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/Survival Analysis On CHR-P_Results/stage4_Timing-difference separation'
...
stage4_spec_version <- "stage4_interval_backbone_v2_20260328"
stage4_caching_policy <- "reuse_existing_outputs_only_when_schema_and_new_interval_backbone_are_valid"
...
save_plot_png <- function(plot_object, filename, width = 11, height = 8.5, dpi = 300) {
  ggplot2::ggsave(
    filename = file.path(export_path, filename),
    plot = plot_object,
    width = width,
    height = height,
    dpi = dpi,
...
# 🔴 Recover: reusable stage components when compatible ===============================
existing_supported_short_file <- file.path(export_path, "stage4_supported_short_horizon_contrast.csv")
existing_piecewise_support_file <- file.path(export_path, "stage4_piecewise_interval_support.csv")
existing_piecewise_site_effect_file <- file.path(export_path, "stage4_piecewise_site_effect.csv")
existing_model_objects_file <- file.path(export_path, "stage4_model_objects.rds")
...
build_log <- append_build_log(
  ...
  status = "assembled",
  source_file = NA_character_,
  detail = "supported_short_horizon_contrast, intermediate_horizon_contrast, and tail_diagnostic_contrast were assembled under the revised Stage 4 labeling contract."
)
...
stage4_metadata_registry <- tibble::tribble(
  ...
  "stage", "canonical_framework", "Integrated_Modeling_Master_Specification_English_REVISED_v5.md",
  "stage", "stage4_spec_version", stage4_spec_version,
  "stage", "stage4_caching_policy", stage4_caching_policy,
  "stage", "stage4_revision_note", "Revised Stage 4 uses the interval backbone 0-1, 1-2, 2-5, and >5, preserves 1y/2y restricted-window contrasts, and saves both a plot-book PDF and separate PNG files.",
  ...
  "comparison", "count_alignment_validation_rule", "Stage 4 stops or falls back to recomputation if supported/intermediate/tail contrast tables do not align with risk_trajectory or piecewise_interval_support.",
  "comparison", "reuse_rule", "Existing files are reused only when they match the revised interval backbone and required schema; otherwise the corresponding component is recomputed.",
  "outputs", "save_folder_rule", "Write all Stage 4 outputs into one export_path folder without creating extra subfolders.",
  "outputs", "plot_png_rule", "Every Stage 4 plot is saved both into a single PDF plot-book and as a separate PNG file generated from the same exported data-frame sources.",
...
plot_filenames <- c(
  "stage4_plot_risk_trajectory.png",
  "stage4_plot_supported_short_horizon_contrast.png",
  "stage4_plot_intermediate_horizon_contrast.png",
  "stage4_plot_tail_diagnostic_contrast.png",
  "stage4_plot_piecewise_interval_support.png",
  "stage4_plot_hazard_pattern_summary.png"
)
...
stage4_export_manifest <- tibble(
  file_name = c(
    ...
    "stage4_intermediate_horizon_contrast.csv",
    "stage4_tail_diagnostic_contrast.csv",
    "stage4_build_log.csv",
  ...
  object_name = c(
    ...
    "stage4_plot_book",
    rep("stage4_plot_png", length(plot_filenames)),
    "stage4_export_manifest"
  ),
  description = c(
    "Dataset-level Stage 4 summary aligned to the current Stage 1 backbone",
    ...
    rep("Standalone PNG version of a Stage 4 plot generated from the same data-frame source as the PDF page", length(plot_filenames)),
...
stage4_model_objects <- list(
  ...
  config = list(
    ...
    stage4_spec_version = stage4_spec_version,
    stage4_caching_policy = stage4_caching_policy
  )
)
...
readr::write_csv(intermediate_horizon_contrast, file.path(export_path, "stage4_intermediate_horizon_contrast.csv"))
readr::write_csv(tail_diagnostic_contrast, file.path(export_path, "stage4_tail_diagnostic_contrast.csv"))
readr::write_csv(build_log, file.path(export_path, "stage4_build_log.csv"))
...
save_plot_png(plot_risk_trajectory_obj, "stage4_plot_risk_trajectory.png")
save_plot_png(plot_supported_short_obj, "stage4_plot_supported_short_horizon_contrast.png")
save_plot_png(plot_intermediate_obj, "stage4_plot_intermediate_horizon_contrast.png")
save_plot_png(plot_tail_obj, "stage4_plot_tail_diagnostic_contrast.png")
save_plot_png(plot_piecewise_support_obj, "stage4_plot_piecewise_interval_support.png")
save_plot_png(plot_hazard_pattern_obj, "stage4_plot_hazard_pattern_summary.png")
