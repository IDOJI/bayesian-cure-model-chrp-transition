...
export_path <- '/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/Survival Analysis On CHR-P_Results/stage1_Backbone lock'
...
code_rules_file <- "Rules Before Generating R Code_🇬🇧ENG.md"
...
derive_claim_restriction_flag <- function(horizon_evidence_class, support_tier) {
  if (is.na(horizon_evidence_class) || is.na(support_tier)) {
...
make_formula_registry <- function() {
  ...
  formula_tbl %>%
    mutate(
      ...
      site_term_interpretation = if_else(
        uses_site,
        "structural_context_proxy_not_causal_treatment_effect",
        "not_applicable"
      )
    ) %>%
    select(
      dataset, dataset_key, formula_id, formula_name, formula_label,
      formula_rhs, formula_full,
      uses_site, uses_age_sex_interaction,
      site_branch, interaction_branch,
      risk_scale, formula_scope, site_term_interpretation
    )
}
...
make_dataset_registry <- function(analysis_datasets, source_lookup) {
  bind_rows(lapply(names(analysis_datasets), function(dataset_name) {
    ...
    tibble::tibble(
      ...
      analysis_structure = ifelse(dataset_name == "merged", "site_free_backbone", "single_site"),
      n_rows = nrow(df),
      ...
      site_effect_role = ifelse(
        dataset_name == "merged",
        "site_terms_are_structural_context_proxies_not_causal_treatment_effects",
        "single_site_dataset"
      )
    )
  }))
...
make_age_summary_block <- function(df, dataset_name) {
  ...
  by_sex <- df %>%
    ...
    select(dataset, dataset_key, summary_domain, group_value, n, proportion, age_mean, age_sd, age_median, age_q1, age_q3, age_min, age_max)
...
make_horizon_registry <- function() {
  tibble::as_tibble(expand.grid(
    ...
    mutate(
      ...
      interpretation_tier = vapply(support_tier, derive_legacy_interpretation_tier, character(1)),
      ...
      claim_restriction_flag = mapply(derive_claim_restriction_flag, horizon_evidence_class, support_tier, USE.NAMES = FALSE),
...
make_threshold_registry <- function() {
  tibble::tibble(
    ...
    applies_to = "all_datasets_all_model_classes_within_risk_scale",
    editable_in_script_config = TRUE,
    threshold_value_type = "risk_probability"
  )
}
...
make_metadata_registry <- function(source_lookup) {
  tibble::tribble(
    ...
    "stage", "stage_name", "Stage 1 backbone lock",
    "stage", "stage_role", "Freeze one common analytic backbone before any model fitting",
    ...
    "identification", "unique_person_id_rule", "site + id",
    ...
    "event", "main_risk_scale", main_risk_scale,
    "event", "supplementary_risk_scale", supplementary_risk_scale,
    "event", "supplementary_competing_event_definition", "status_num == 2",
    "event", "supplementary_right_censoring_definition", "status_num == 0",
    "covariates", "age_variable", "age_exact_entry",
    "covariates", "age_scaling_rule", "(age_exact_entry - mean(age_exact_entry)) / (2 * sd(age_exact_entry))",
    "horizons", "horizon_vector", horizon_string,
    "horizons", "horizon_rule", "Fixed to 1:10 in Stage 1 and used as the common annual reporting grid",
    "thresholds", "threshold_vector", threshold_string,
    "thresholds", "threshold_edit_rule", "Edit `risk_thresholds` near the top of the script if needed",
    "interpretation", "site_effect_interpretation_rule", "Interpret site as a proxy for broader treatment context, care pathway, referral structure, selection, or follow-up maturity.",
    "interpretation", "site_effect_causal_warning", "Do not interpret merged site terms as clean causal treatment effects.",
    "interpretation", "risk_scale_comparison_rule", "Never compare models evaluated on incompatible risk scales as if the comparison were fair.",
    "interpretation", "supported_horizon_hierarchy_rule", "PNU: 1y primary, 2y sensitivity, 3+y projection; SNU/merged: 1-2y primary, 3-5y secondary, 6+y projection.",
    "interpretation", "remission_reanalysis_note", "Remission-sensitive competing-risk reanalysis belongs to later stages and remains supplementary to the main transition-only scale.",
    "outputs", "save_folder_rule", "Write all outputs into one `export_path` folder without creating extra subfolders.",
    "inputs", "data_path", data_path,
    "inputs", "export_path", export_path,
    "inputs", "merged_source", unname(source_lookup[["merged"]]),
    "inputs", "pnu_source", unname(source_lookup[["PNU"]]),
    "inputs", "snu_source", unname(source_lookup[["SNU"]])
  )
}
...
make_modeling_registry <- function(formula_registry, dataset_registry) {
  formula_registry %>%
    left_join(
      dataset_registry %>%
        select(dataset, dataset_key, n_site_levels, site_values, main_risk_scale, supplementary_risk_scale, site_effect_role),
      by = c("dataset", "dataset_key")
    ) %>%
    mutate(
      stage = "Stage 1",
    ...
    select(
      ...
      risk_scale, horizon_vector, threshold_vector,
      intended_model_classes, site_term_interpretation, site_effect_role,
      stage, stage_role
    )
}
...
backbone_bundle <- list(
  ...
  config = list(
    ...
    shared_master_spec_file = shared_master_spec_file,
    bayesian_companion_spec_file = bayesian_companion_spec_file,
    code_rules_file = code_rules_file,
...
export_descriptions <- c(
  "Dataset-level Stage 1 registry with dataset keys, analysis structure, and risk-scale metadata",
  ...
  "Common formula registry for the main transition-only scale",
  "Common modeling registry table for downstream model blocks",
  "Dataset-by-horizon support, evidence, and claim-restriction registry",
  "User-editable threshold registry",
  "Stage 1 backbone metadata aligned to the revised integrated master specification",
