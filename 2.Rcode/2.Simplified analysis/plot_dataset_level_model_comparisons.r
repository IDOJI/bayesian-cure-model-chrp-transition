...
find_repo_root <- function(start_dir) {
  ...
  repeat {
    simplified_reference <- file.path(
      ...
      "2.Rcode",
      "Simplified",
      "integrate_simple_model_risk_probabilities.r"
    )
  ...
  stop(
    paste(
      ...
      "`2.Rcode/Simplified/integrate_simple_model_risk_probabilities.r`."
    ),
    call. = FALSE
  )
}
...
export_path <- Sys.getenv(
  ...
  unset = file.path(results_root, "dataset_level_model_comparisons")
)
...
stage7_curve_file <- file.path(results_root, "stage7_simple_lognormal_mixture_cure_plot_source.csv")
stage8_curve_file <- file.path(results_root, "stage8A_simple_bayesian_transition_only_curve_data.csv")
integrated_risk_dir <- file.path(results_root, "integrated_risk_probabilities_by_horizon")
