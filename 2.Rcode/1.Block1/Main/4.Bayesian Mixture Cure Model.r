...
export_path <- Sys.getenv(
  ...
  unset = file.path(block1_root_default, "4.Bayesian Mixture Cure")
)
...
output_prefix <- Sys.getenv(
  "STAGE8A_SIMPLE_OUTPUT_PREFIX",
  unset = "bayesian_mixture_cure"
)
analysis_label <- Sys.getenv(
  "STAGE8A_SIMPLE_ANALYSIS_LABEL",
  unset = "Bayesian mixture cure"
)
...
stan_adapt_delta <- as.numeric(Sys.getenv("STAGE8A_SIMPLE_STAN_ADAPT_DELTA", unset = "0.99"))
...
latency_plot_source_file <- file.path(export_path, paste0(output_prefix, "_latency_plot_source.csv"))
parameter_summary_file <- file.path(export_path, paste0(output_prefix, "_parameter_summary.csv"))
posterior_draw_summary_file <- file.path(export_path, paste0(output_prefix, "_posterior_draw_summary.csv"))
uncured_latency_summary_file <- file.path(export_path, paste0(output_prefix, "_uncured_latency_summary.csv"))
latency_aft_effect_summary_file <- file.path(export_path, paste0(output_prefix, "_latency_aft_effect_summary.csv"))
standard_error_registry_file <- file.path(export_path, paste0(output_prefix, "_standard_error_table_registry.csv"))
standard_error_long_file <- file.path(export_path, paste0(output_prefix, "_standard_error_long.csv"))
prior_branch_horizon_summary_file <- file.path(export_path, paste0(output_prefix, "_prior_branch_horizon_summary.csv"))
prior_branch_delta_file <- file.path(export_path, paste0(output_prefix, "_anchor_vs_neutral_delta_summary.csv"))
...
support_subdir <- file.path(export_path, "sub")
fit_rds_path <- function(model_id, export_dir = export_path) {
  file.path(export_dir, "sub", paste0(output_prefix, "__", model_id, "__fit.rds"))
}
...
summarize_posterior_draws <- function(x) {
  tibble(
    posterior_mean = mean(x, na.rm = TRUE),
    posterior_sd = safe_sd(x),
    q025 = as.numeric(stats::quantile(x, probs = 0.025, na.rm = TRUE, names = FALSE, type = 8)),
    q50 = as.numeric(stats::quantile(x, probs = 0.50, na.rm = TRUE, names = FALSE, type = 8)),
    q975 = as.numeric(stats::quantile(x, probs = 0.975, na.rm = TRUE, names = FALSE, type = 8))
  )
}

summarize_probability_draws <- function(x, prefix) {
  out <- summarize_posterior_draws(x)
  names(out) <- c(
    prefix,
    paste0(prefix, "_sd"),
    paste0(prefix, "_q025"),
...
make_latency_term_label <- function(term_name) {
  dplyr::case_when(
    identical(term_name, "age_s") ~ "Age (per +2 SD on scaled age_s)",
    identical(term_name, "sex_num") ~ "Male vs Female",
    identical(term_name, "site_SNU") ~ "SNU vs PNU",
    TRUE ~ as.character(term_name)
  )
}

solve_uncured_mixture_median_year <- function(mu_vec, sigma_value) {
  objective <- function(log_time_value) {
    mean(stats::pnorm((log_time_value - mu_vec) / sigma_value)) - 0.5
  }
  ...
  bind_rows(
    ...
    base_tbl %>%
      mutate(
        ...
        model_id = paste0(model_id, "__neutral")
      )
  )
}
...
make_design_bundle <- function(df, model_row, prior_artifacts, snu_label) {
  ...
  prior_branch <- as.character(model_row$prior_branch[[1L]])

  if (identical(prior_branch, "anchor_informed")) {
  ...
  } else if (identical(prior_branch, "neutral_no_external_info")) {
    mu_beta_inc <- rep(0, ncol(X_inc))
    sd_beta_inc <- rep(prior_artifacts$neutral_beta_inc_sd, ncol(X_inc))
    alpha_inc_prior_mean <- 0
    alpha_inc_prior_sd <- as.numeric(prior_artifacts$alpha_inc_sd)
  } else {
    stop("Unknown prior branch: ", prior_branch, call. = FALSE)
  }
  ...
  list(
    ...
    alpha_inc_prior_mean = alpha_inc_prior_mean,
    alpha_inc_prior_sd = alpha_inc_prior_sd,
...
build_posterior_draw_summary_df <- function(prior_branch_horizon_summary_df) {
  estimand_registry <- tibble::tribble(
    ~estimand, ~estimand_label,
    "susceptible_fraction", "Susceptible fraction",
    "cure_fraction", "Cure fraction",
    "overall_survival_prob", "Overall survival probability",
    "overall_risk_prob", "Overall risk probability",
    "susceptible_only_survival_prob", "Susceptible-only survival probability",
    "susceptible_only_risk_prob", "Susceptible-only risk probability"
  )

  bind_rows(lapply(seq_len(nrow(estimand_registry)), function(ii) {
    estimand_name <- estimand_registry$estimand[[ii]]

    tibble(
      dataset = prior_branch_horizon_summary_df$dataset,
      dataset_label = prior_branch_horizon_summary_df$dataset_label,
      plot_dataset_label = prior_branch_horizon_summary_df$plot_dataset_label,
      model_id = prior_branch_horizon_summary_df$model_id,
      prior_branch = prior_branch_horizon_summary_df$prior_branch,
      site_adjustment_flag = prior_branch_horizon_summary_df$site_adjustment_flag,
      risk_scale = prior_branch_horizon_summary_df$risk_scale,
      horizon_year = prior_branch_horizon_summary_df$horizon_year,
      estimand = estimand_name,
      estimand_label = estimand_registry$estimand_label[[ii]],
      posterior_mean = prior_branch_horizon_summary_df[[estimand_name]],
      posterior_sd = prior_branch_horizon_summary_df[[paste0(estimand_name, "_sd")]],
      posterior_q025 = prior_branch_horizon_summary_df[[paste0(estimand_name, "_q025")]],
      posterior_q50 = prior_branch_horizon_summary_df[[paste0(estimand_name, "_q50")]],
      posterior_q975 = prior_branch_horizon_summary_df[[paste0(estimand_name, "_q975")]],
      n_subjects = prior_branch_horizon_summary_df$n_subjects
    )
  })) %>%
    arrange(
      ...
      prior_branch,
      horizon_year,
      match(estimand, estimand_registry$estimand)
    )
}
...
build_curve_prediction_df_from_draws <- function(model_row, design_bundle, draw_bundle, times) {
  ...
  bind_rows(lapply(times, function(tt) {
    susceptible_fraction_draws <- colMeans(pi_mat)
    cure_fraction_draws <- 1 - susceptible_fraction_draws

    if (tt <= 0) {
      susceptible_only_survival_draws <- rep(1, draw_bundle$n_draws)
      ...
      susceptible_only_survival_draws <- colMeans(susceptible_only_survival_draws)
      overall_survival_draws <- colMeans(overall_survival_draws)
    }

    overall_risk_draws <- 1 - overall_survival_draws
    susceptible_only_risk_draws <- 1 - susceptible_only_survival_draws
    ...
    bind_cols(
      ...
      summarize_probability_draws(susceptible_fraction_draws, "susceptible_fraction"),
      summarize_probability_draws(cure_fraction_draws, "cure_fraction"),
      summarize_probability_draws(overall_survival_draws, "overall_survival_prob"),
      summarize_probability_draws(overall_risk_draws, "overall_risk_prob"),
      summarize_probability_draws(susceptible_only_survival_draws, "susceptible_only_survival_prob"),
      summarize_probability_draws(susceptible_only_risk_draws, "susceptible_only_risk_prob")
    )
  }))
}

build_uncured_latency_summary_df_from_draws <- function(model_row, design_bundle, draw_bundle) {
  X_lat <- design_bundle$X_lat
  mu_lat_mat <- X_lat %*% t(draw_bundle$gamma_lat)
  mu_lat_mat <- sweep(mu_lat_mat, 2, draw_bundle$gamma0, "+")
  sigma_vec <- exp(draw_bundle$log_sigma)

  mean_time_draws <- vapply(seq_len(draw_bundle$n_draws), function(jj) {
    mean(exp(mu_lat_mat[, jj] + 0.5 * sigma_vec[[jj]]^2), na.rm = TRUE)
  }, numeric(1))

  median_time_draws <- vapply(seq_len(draw_bundle$n_draws), function(jj) {
    solve_uncured_mixture_median_year(mu_lat_mat[, jj], sigma_vec[[jj]])
  }, numeric(1))

  bind_rows(
    tibble(
      metric = "uncured_latency_mean_time_to_transition_years",
      metric_label = "Mean time to transition among uncured subgroup (years)"
    ) %>%
      bind_cols(summarize_posterior_draws(mean_time_draws)),
    tibble(
      metric = "uncured_latency_median_time_to_transition_years",
      metric_label = "Median time to transition among uncured subgroup (years)"
    ) %>%
      bind_cols(summarize_posterior_draws(median_time_draws))
  ) %>%
    mutate(
      dataset = model_row$dataset[[1L]],
      dataset_label = model_row$dataset_label[[1L]],
      ...
      structural_model_id = model_row$structural_model_id[[1L]],
      formula_anchor = model_row$formula_anchor[[1L]],
      summary_level = "cohort_level_conditional_on_uncured",
      time_unit = "years",
      n_subjects = nrow(X_lat),
      n_posterior_draws = as.integer(draw_bundle$n_draws),
      .before = 1
    )
}

build_latency_aft_effect_summary_df_from_draws <- function(model_row, design_bundle, draw_bundle) {
  latency_term_names <- colnames(design_bundle$X_lat)
  if (is.null(latency_term_names)) {
    latency_term_names <- paste0("gamma_lat_", seq_len(ncol(design_bundle$X_lat)))
  }

  bind_rows(lapply(seq_along(latency_term_names), function(jj) {
    term_name <- latency_term_names[[jj]]
    effect_draws <- exp(draw_bundle$gamma_lat[, jj])
    ...
    tibble(
      ...
      structural_model_id = model_row$structural_model_id[[1L]],
      formula_anchor = model_row$formula_anchor[[1L]],
      latency_term = term_name,
      latency_term_label = make_latency_term_label(term_name),
      effect_scale = "AFT_time_ratio",
      effect_interpretation = "Multiplicative change in uncured time-to-transition for a one-unit increase in the latency covariate",
      n_posterior_draws = as.integer(draw_bundle$n_draws)
    ) %>%
      bind_cols(summarize_posterior_draws(effect_draws))
  }))
}
...
cleanup_existing_stage8_outputs <- function(export_dir) {
  if (!dir.exists(export_dir)) {
  ...
  stale_files <- list.files(
    export_dir,
    pattern = paste0("^", output_prefix, "_.*\\.(csv|rds|png|pdf)$"),
    full.names = TRUE
  )
  stale_files <- stale_files[!grepl(paste0("^", output_prefix, "__.*__fit\\.rds$"), basename(stale_files))]
  legacy_fit_files <- list.files(
    export_dir,
    pattern = paste0("^", output_prefix, "__.*__fit\\.rds$"),
    full.names = TRUE
  )
  sub_support_files <- character()
  if (dir.exists(file.path(export_dir, "sub"))) {
    sub_support_files <- list.files(
      file.path(export_dir, "sub"),
      pattern = paste0("^", output_prefix, "_.*\\.(csv|rds|png|pdf)$"),
      full.names = TRUE
    )
    sub_support_files <- sub_support_files[
      !grepl(paste0("^", output_prefix, "__.*__fit\\.rds$"), basename(sub_support_files))
    ]
  }
  stale_files <- unique(c(stale_files, legacy_fit_files, sub_support_files))
  ...
  if (any(!removed_flag)) {
    stop(
      sprintf(
        "Failed to remove stale Bayesian outputs: %s",
...
organize_stage8_outputs <- function(output_dir) {
  ...
  keep_files <- c(
    ...
    paste0(output_prefix, "_prior_branch_horizon_summary.csv"),
    paste0(output_prefix, "_anchor_vs_neutral_delta_summary.csv"),
    paste0(output_prefix, "_plot_source.csv"),
    paste0(output_prefix, "_detail_annotation_table.csv"),
    paste0(output_prefix, "_model_registry.csv"),
    paste0(output_prefix, "_latency_plot_source.csv"),
    paste0(output_prefix, "_parameter_summary.csv"),
    paste0(output_prefix, "_posterior_draw_summary.csv"),
    paste0(output_prefix, "_uncured_latency_summary.csv"),
    paste0(output_prefix, "_latency_aft_effect_summary.csv"),
    paste0(output_prefix, "_standard_error_table_registry.csv"),
    paste0(output_prefix, "_standard_error_long.csv"),
    paste0(output_prefix, "_estimated_survival_curve__all_cohorts.png"),
    paste0(output_prefix, "_estimated_risk_curve__all_cohorts.png")
  )
  ...
  if (any(!moved_flag)) {
    stop(
      sprintf(
        "Failed to move Bayesian support files into `sub`: %s",
...
make_curve_plot <- function(
  ...
  subtitle_text <- "Cohort-level posterior means with 95% credible intervals from anchored Bayesian log-normal mixture cure fits"
...
uncured_latency_summary_list <- vector("list", nrow(model_spec_registry))
latency_aft_effect_summary_list <- vector("list", nrow(model_spec_registry))
...
for (ii in seq_len(nrow(model_spec_registry))) {
  ...
  if (is.null(fit)) {
    ...
    dir.create(dirname(one_fit_rds_path), recursive = TRUE, showWarnings = FALSE)
  ...
  uncured_latency_summary_list[[ii]] <- build_uncured_latency_summary_df_from_draws(
    model_row = model_row,
    design_bundle = design_bundle,
    draw_bundle = draw_bundle
  )
  latency_aft_effect_summary_list[[ii]] <- build_latency_aft_effect_summary_df_from_draws(
    model_row = model_row,
    design_bundle = design_bundle,
    draw_bundle = draw_bundle
  )
...
curve_data_df <- bind_rows(curve_data_list) %>%
  ...
  arrange(match(dataset, dataset_order), prior_branch, time_horizon_year)

parameter_summary_df <- bind_rows(parameter_summary_list) %>%
  arrange(match(dataset, dataset_order), prior_branch, model_id, parameter)

uncured_latency_summary_df <- bind_rows(uncured_latency_summary_list) %>%
  arrange(match(dataset, dataset_order), prior_branch, model_id, metric)

latency_aft_effect_summary_df <- bind_rows(latency_aft_effect_summary_list) %>%
  arrange(match(dataset, dataset_order), prior_branch, model_id, latency_term)

model_registry_df <- bind_rows(model_registry_list) %>%
  arrange(match(dataset, dataset_order), prior_branch, model_id)

prior_branch_horizon_summary_df <- curve_data_df %>%
  filter(time_horizon_year %in% horizon_years) %>%
  transmute(
    dataset,
    dataset_label,
  ...
  arrange(match(dataset, dataset_order), prior_branch, horizon_year)
...
prior_branch_delta_df <- prior_branch_horizon_summary_df %>%
  group_by(dataset, dataset_label, horizon_year) %>%
  summarise(
    anchor_overall_risk_prob = overall_risk_prob[prior_branch == "anchor_informed"][[1L]],
    neutral_overall_risk_prob = overall_risk_prob[prior_branch == "neutral_no_external_info"][[1L]],
    delta_risk_anchor_minus_neutral = anchor_overall_risk_prob - neutral_overall_risk_prob,
    anchor_cure_fraction = cure_fraction[prior_branch == "anchor_informed"][[1L]],
    neutral_cure_fraction = cure_fraction[prior_branch == "neutral_no_external_info"][[1L]],
    delta_cure_fraction_anchor_minus_neutral = anchor_cure_fraction - neutral_cure_fraction,
    .groups = "drop"
  ) %>%
  arrange(match(dataset, dataset_order), horizon_year)

posterior_draw_summary_df <- build_posterior_draw_summary_df(prior_branch_horizon_summary_df)
...
cleanup_existing_stage8_outputs(export_path)
...
latency_plot_source_df <- bind_rows(
  uncured_latency_summary_df %>%
    mutate(component_type = "uncured_latency_summary"),
  latency_aft_effect_summary_df %>%
    mutate(component_type = "latency_aft_effect_summary")
) %>%
  mutate(
    component_type = factor(
      component_type,
      levels = c("uncured_latency_summary", "latency_aft_effect_summary")
    )
  ) %>%
  arrange(match(dataset, dataset_order), component_type) %>%
...
standard_error_export_bundle <- build_standard_error_export_bundle(list(
  ...
  prior_branch_horizon_summary = prior_branch_horizon_summary_df,
  prior_branch_delta = prior_branch_delta_df,
  ...
  posterior_draw_summary = posterior_draw_summary_df,
  uncured_latency_summary = uncured_latency_summary_df,
  latency_aft_effect_summary = latency_aft_effect_summary_df
))

# Write Outputs -----------------------------------------------------------
readr::write_csv(horizon_summary_df, horizon_summary_file)
readr::write_csv(prior_branch_horizon_summary_df, prior_branch_horizon_summary_file)
readr::write_csv(prior_branch_delta_df, prior_branch_delta_file)
...
readr::write_csv(posterior_draw_summary_df, posterior_draw_summary_file)
readr::write_csv(uncured_latency_summary_df, uncured_latency_summary_file)
readr::write_csv(latency_aft_effect_summary_df, latency_aft_effect_summary_file)
...
organize_stage8_outputs(export_path)
...
message("Prior-branch horizon summary CSV: ", normalizePath(prior_branch_horizon_summary_file, winslash = "/", mustWork = FALSE))
message("Anchor-vs-neutral delta CSV: ", normalizePath(prior_branch_delta_file, winslash = "/", mustWork = FALSE))
...
message("Posterior draw summary CSV: ", normalizePath(posterior_draw_summary_file, winslash = "/", mustWork = FALSE))
message("Uncured latency summary CSV: ", normalizePath(uncured_latency_summary_file, winslash = "/", mustWork = FALSE))
message("Latency AFT effect summary CSV: ", normalizePath(latency_aft_effect_summary_file, winslash = "/", mustWork = FALSE))
...
message("Per-model fit RDS files are stored in: ", normalizePath(support_subdir, winslash = "/", mustWork = FALSE))
