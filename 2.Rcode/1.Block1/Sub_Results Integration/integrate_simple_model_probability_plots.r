...
export_path <- Sys.getenv(
  ...
  unset = file.path(results_integration_root, "integrated_model_probability_plots")
)
...
stage7_plot_source_candidates <- c(
...
stage8_plot_source_candidates <- c(
...
# Load Packages -----------------------------------------------------------
required_packages <- c("ggplot2", "dplyr", "readr", "tibble", "scales")
...
cleanup_existing_probability_plot_outputs <- function(export_dir) {
...
extract_point_layer_data <- function(plot_object, dataset_col, point_time_col, value_col, label) {
  ...
  candidate_layers <- Filter(
    function(df) all(c(dataset_col, point_time_col, value_col) %in% names(df)),
    candidate_layers
  )

  if (length(candidate_layers) == 0L) {
    stop(sprintf("%s does not contain a compatible point layer.", label), call. = FALSE)
  }

  is_horizon_layer <- vapply(candidate_layers, function(df) {
    time_values <- suppressWarnings(as.numeric(df[[point_time_col]]))
    ...
    identical(sort(unique(as.integer(round(time_values)))), as.integer(horizon_years))
  }, logical(1))

  if (any(is_horizon_layer)) {
    return(candidate_layers[[which(is_horizon_layer)[[1L]]]])
  }
...
extract_plot_curve_and_points <- function(
  ...
  point_df <- extract_point_layer_data(
    plot_object = plot_object,
    dataset_col = dataset_col,
    point_time_col = point_time_col,
    value_col = value_col,
    label = label
  )
...
extract_curve_and_points_from_source <- function(
  source_df,
  dataset_col,
  time_col,
  value_col,
  ...
  point_out <- source_df %>%
    transmute(
      ...
      estimate_scope = estimate_scope,
      time_year = as.numeric(.data[[time_col]]),
      horizon_year = as.integer(round(as.numeric(.data[[time_col]]))),
      estimate_value = clip_probability(.data[[value_col]])
    ) %>%
    filter(
      !is.na(dataset_key),
      ...
      !is.na(horizon_year),
      !is.na(estimate_value),
      abs(time_year - horizon_year) < 1e-8,
      horizon_year %in% horizon_years
    ) %>%
    select(-time_year) %>%
    distinct(dataset_key, dataset_label, model_name, estimate_scope, horizon_year, .keep_all = TRUE)
...
build_plot_sources <- function() {
  ...
  stage7_plot_source <- read_csv_checked(stage7_plot_source_file, "Stage 7 plot source CSV")
  stage8_plot_source <- read_csv_checked(stage8_plot_source_file, "Stage 8A plot source CSV")
  stage7_time_col <- if ("time_horizon_year" %in% names(stage7_plot_source)) "time_horizon_year" else "time_year"
  stage8_time_col <- if ("time_year" %in% names(stage8_plot_source)) "time_year" else "time_horizon_year"

  if ("prior_branch" %in% names(stage8_plot_source)) {
    stage8_plot_source <- stage8_plot_source %>%
  ...
  list(
    overall_survival = list(
      ...
      stage7 = extract_curve_and_points_from_source(
        source_df = stage7_plot_source,
        dataset_col = "dataset",
        time_col = stage7_time_col,
        value_col = "overall_survival_prob",
        model_name = "Frequentist mixture cure (lognormal)",
        estimate_scope = "Overall cohort reference"
      ),
      stage8 = extract_curve_and_points_from_source(
        source_df = stage8_plot_source,
        dataset_col = "dataset",
        time_col = stage8_time_col,
        value_col = "overall_survival_prob",
...
build_probability_plot_source_table <- function(plot_sources) {
  plot_family_lookup <- c(
    overall_survival = "overall_survival_probability",
    overall_risk = "overall_risk_probability",
    susceptible_survival = "susceptible_survival_probability",
    susceptible_risk = "susceptible_risk_probability"
  )
  metric_lookup <- c(
    overall_survival = "survival",
    overall_risk = "risk",
    susceptible_survival = "survival",
    susceptible_risk = "risk"
  )
  plot_family_order <- unname(plot_family_lookup[c(
    "overall_survival",
    "overall_risk",
    "susceptible_survival",
    "susceptible_risk"
  )])

  bind_rows(lapply(names(plot_sources), function(plot_key) {
    curve_df <- collapse_sources(plot_sources[[plot_key]], "curve") %>%
      transmute(
        plot_family = unname(plot_family_lookup[[plot_key]]),
        metric = unname(metric_lookup[[plot_key]]),
        component_type = "curve",
        dataset = as.character(dataset_key),
        dataset_label = as.character(dataset_label),
        model = as.character(model_name),
        estimate_scope = as.character(estimate_scope),
        draw_style = as.character(draw_style),
        time_year = as.numeric(time_year),
        horizon_year = NA_integer_,
        estimate_probability = as.numeric(estimate_value)
      )

    point_df <- collapse_sources(plot_sources[[plot_key]], "points") %>%
      transmute(
        plot_family = unname(plot_family_lookup[[plot_key]]),
        metric = unname(metric_lookup[[plot_key]]),
        ...
        model = as.character(model_name),
        estimate_scope = as.character(estimate_scope),
        draw_style = NA_character_,
        time_year = as.numeric(horizon_year),
        horizon_year = as.integer(horizon_year),
        estimate_probability = as.numeric(estimate_value)
      )

    bind_rows(curve_df, point_df)
  })) %>%
    mutate(
      .plot_order = match(plot_family, plot_family_order),
      .dataset_order = match(dataset, dataset_order),
      .model_order = match(model, model_order),
      .component_order = match(component_type, c("curve", "yearly_point")),
      .scope_order = match(estimate_scope, scope_order),
      .time_order = dplyr::coalesce(time_year, as.numeric(horizon_year))
    ) %>%
    arrange(.plot_order, .dataset_order, .model_order, .scope_order, .component_order, .time_order) %>%
    select(-starts_with("."))
}
...
stage7_plot_source_file <- resolve_existing_path(
  stage7_plot_source_candidates,
  "Stage 7 plot source CSV"
)
stage8_plot_source_file <- resolve_existing_path(
  stage8_plot_source_candidates,
  "Stage 8A plot source CSV"
)

plot_sources <- build_plot_sources()
cleanup_existing_probability_plot_outputs(export_path)
integrated_plot_source_table <- build_probability_plot_source_table(plot_sources)

readr::write_csv(
  integrated_plot_source_table,
  file.path(export_path, "integrated_model_probability_plot_source.csv")
)
