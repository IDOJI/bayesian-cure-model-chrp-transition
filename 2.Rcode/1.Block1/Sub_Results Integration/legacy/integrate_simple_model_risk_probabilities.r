...
export_path <- Sys.getenv(
  ...
  unset = file.path(results_integration_root, "integrated_risk_probabilities_by_horizon")
)

stage3_input_candidates <- c(
  file.path(modeling_root, "1.KM", "simple_km_yearly_estimates.csv"),
  file.path(results_root, "stage3_simple_km_yearly_estimates.csv")
)
stage5_input_candidates <- c(
  file.path(modeling_root, "2.MLE No-Cure", "mle_nocure_lognormal_horizon_summary.csv"),
  file.path(modeling_root, "2.MLE No-Cure", "stage5_simple_lognormal_horizon_summary.csv"),
  file.path(
    results_root,
    "stage5_individualized_nocure_lognormal_simple",
    "stage5_simple_lognormal_horizon_summary.csv"
  )
)
stage7_input_candidates <- c(
  file.path(modeling_root, "3.MLE Mixture Cure", "mle_mixture_cure_lognormal_horizon_summary.csv"),
  file.path(modeling_root, "3.MLE Mixture Cure", "stage7_simple_lognormal_mixture_cure_horizon_summary.csv"),
...
cleanup_existing_risk_outputs <- function(export_dir) {
...
standardize_stage8 <- function(path) {
  ...
  horizon_col <- if ("horizon_year" %in% names(stage8_df)) "horizon_year" else "time_horizon_year"
  ...
  bind_rows(
    stage8_df %>%
      transmute(
        ...
        horizon_year = as.integer(.data[[horizon_col]]),
        estimated_risk_probability = clip_probability(overall_risk_prob)
      ),
    stage8_df %>%
      transmute(
...
# Execute Integration -----------------------------------------------------
stage3_input_file <- resolve_existing_path(stage3_input_candidates, "Stage 3 yearly estimate file")
stage5_input_file <- resolve_existing_path(stage5_input_candidates, "Stage 5 horizon summary file")
stage7_input_file <- resolve_existing_path(stage7_input_candidates, "Stage 7 horizon summary file")
stage8_input_file <- resolve_existing_path(stage8_input_candidates, "Stage 8A horizon summary file")
...
cleanup_existing_risk_outputs(export_path)
