...
nocure_input_path <- Sys.getenv(
  "STAGE5_SIMPLE_EXPORT_PATH",
  unset = file.path(block1_root_default, "2.MLE No-Cure")
)
...
nocure_horizon_summary_file <- file.path(nocure_input_path, "mle_nocure_lognormal_horizon_summary.csv")
nocure_survival_plot_file <- file.path(nocure_input_path, "mle_nocure_lognormal_survival_plot.rds")
nocure_risk_plot_file <- file.path(nocure_input_path, "mle_nocure_lognormal_risk_plot.rds")
nocure_detail_bundle_file <- file.path(nocure_input_path, "sub", "mle_nocure_lognormal_detail_plot_bundle.rds")
nocure_fitted_models_file <- file.path(nocure_input_path, "mle_nocure_lognormal_fitted_models.rds")
cure_horizon_summary_file <- file.path(cure_input_path, "mle_mixture_cure_lognormal_horizon_summary.csv")
cure_plot_source_file <- file.path(cure_input_path, "mle_mixture_cure_lognormal_plot_source.csv")
bayesian_horizon_summary_file <- file.path(bayesian_input_path, "bayesian_mixture_cure_horizon_summary.csv")
bayesian_plot_source_file <- file.path(bayesian_input_path, "bayesian_mixture_cure_plot_source.csv")
...
model_order <- c("KM", "MLE no-cure", "MLE cure", "Bayesian cure")
model_palette <- c(
  "KM" = "#1B4332",
  "MLE no-cure" = "#D97706",
  "MLE cure" = "#C1666B",
  "Bayesian cure" = "#2C7FB8"
)
model_linetypes <- c(
  "KM" = "solid",
  "MLE no-cure" = "longdash",
  "MLE cure" = "22",
  "Bayesian cure" = "dotdash"
)
...
cleanup_existing_integrated_outputs <- function(output_dir) {
  ...
  if (any(!removed_flag)) {
    stop(
      sprintf(
        "Failed to remove stale integrated KM-vs-no-cure-vs-cure outputs: %s",
...
prepare_merged_analysis_dataset <- function(source_file) {
  ...
  missing_cols <- setdiff(required_cols, names(source_df))
  if (length(missing_cols) > 0L) {
    stop(
      sprintf("Merged source data is missing required columns: %s", paste(missing_cols, collapse = ", ")),
      call. = FALSE
    )
  }
  ...
  out %>%
    mutate(
      unique_person_id = paste(as.character(site), id, sep = "_"),
      time_year = days_followup / 365.25,
      time_year_model = pmax(time_year, 1e-08),
...
build_nocure_site_adjusted_from_fit <- function(fitted_models, merged_df) {
  fit <- fitted_models[["merged_site_adjusted"]]
  if (!inherits(fit, "flexsurvreg")) {
    stop("No-cure fitted model for `merged_site_adjusted` is unavailable.", call. = FALSE)
  }

  curve_times <- seq(curve_step_year, curve_horizon_max_year, by = curve_step_year)
  yearly_survival <- clip_probability(colMeans(
    predict_parametric_survival(fit, merged_df, horizon_years),
    na.rm = TRUE
  ))
  curve_survival <- clip_probability(colMeans(
    predict_parametric_survival(fit, merged_df, curve_times),
    na.rm = TRUE
  ))
  ...
  list(
    curve = tibble(
      ...
      estimate_value = c(1, curve_survival)
    ),
    yearly = tibble(
      dataset = "merged_site_adjusted",
      horizon_year = horizon_years,
      estimate_value = yearly_survival
    )
  )
}
...
extract_nocure_curve_data <- function(plot_object, value_col) {
  if (!inherits(plot_object, "ggplot")) {
    stop("No-cure plot object must be a ggplot object.", call. = FALSE)
  }
  ...
  yearly_df <- extract_point_layer_df(
    ...
    time_col = "horizon_year",
    value_col = value_col,
    label = "No-cure plot object"
  )

  list(
    curve = curve_df %>%
      transmute(
        dataset = normalize_dataset_name(dataset_version_key),
        time_year = as.numeric(time_year),
        estimate_value = clip_probability(.data[[value_col]])
      ) %>%
      filter(dataset %in% dataset_order, time_year >= 0, time_year <= curve_horizon_max_year),
...
prepare_cure_curve_data <- function(plot_source_df, horizon_summary_df, value_col) {
...
prepare_bayesian_curve_data <- function(plot_source_df, horizon_summary_df, value_col) {
  plot_source_filtered <- plot_source_df
  horizon_summary_filtered <- horizon_summary_df

  if ("prior_branch" %in% names(plot_source_filtered)) {
    plot_source_filtered <- plot_source_filtered %>%
      filter(prior_branch == "anchor_informed")
  }

  if ("prior_branch" %in% names(horizon_summary_filtered)) {
    horizon_summary_filtered <- horizon_summary_filtered %>%
      filter(prior_branch == "anchor_informed")
  }

  prepare_cure_curve_data(
    plot_source_df = plot_source_filtered,
    horizon_summary_df = horizon_summary_filtered,
    value_col = value_col
  )
}

prepare_followup_annotation <- function(detail_bundle) {
  followup_list <- detail_bundle$followup_summary

  bind_rows(
    lapply(names(followup_list), function(key) {
      followup_list[[key]] %>%
        transmute(
...
build_estimate_panel <- function(estimate_df, metric_label) {
  ...
  ggplot(plot_df, aes(x = horizon_year, y = model)) +
    ...
    geom_text(aes(label = scales::percent(estimate_value, accuracy = 0.1)), size = 3.0, fontface = "bold") +
...
build_curve_panel <- function(curve_df, yearly_df, dataset_name, dataset_summary, metric) {
  ...
  present_models <- unique(as.character(stats::na.omit(plot_df$model)))
  includes_bayesian <- "Bayesian cure" %in% present_models
  ...
  title_text <- if (identical(metric, "survival")) {
    paste0("KM vs MLE no-cure vs MLE cure survival curves - ", dataset_label)
  } else if (includes_bayesian) {
    paste0("KM vs MLE no-cure vs MLE cure vs Bayesian cure cumulative risk curves - ", dataset_label)
  } else {
    paste0("KM vs MLE no-cure vs MLE cure cumulative risk curves - ", dataset_label)
  }

  subtitle_text <- if (includes_bayesian) {
    paste(
      "KM from benchmark RDS, no-cure from frequentist log-normal exports,",
      "cure from frequentist log-normal mixture-cure exports,",
      "and Bayesian cure from Bayesian log-normal mixture-cure exports"
    )
  } else {
    paste(
      "KM from benchmark RDS, no-cure from frequentist log-normal exports,",
      "and cure from frequentist log-normal mixture-cure exports"
    )
  }
  if (identical(dataset_name, "PNU") && is.finite(pnu_followup_year)) {
    subtitle_text <- paste0(
      subtitle_text,
      sprintf(" | Dashed vertical line = PNU max observed follow-up (%.2f years)", pnu_followup_year)
    )
  }
...
build_combined_plot <- function(
  ...
  curve_panel / estimate_panel / count_panels$at_risk_plot / count_panels$transition_plot +
    patchwork::plot_layout(heights = c(3.6, 1.45, 1.4, 1.4))
}
...
nocure_horizon_summary <- read_csv_checked(nocure_horizon_summary_file, "No-cure horizon summary")
nocure_survival_plot <- read_rds_checked(nocure_survival_plot_file, "No-cure survival plot RDS")
nocure_risk_plot <- read_rds_checked(nocure_risk_plot_file, "No-cure risk plot RDS")
nocure_detail_bundle <- read_rds_checked(nocure_detail_bundle_file, "No-cure detail plot bundle")
nocure_fitted_models <- read_rds_checked(nocure_fitted_models_file, "No-cure fitted models RDS")
cure_horizon_summary <- read_csv_checked(cure_horizon_summary_file, "Cure horizon summary")
cure_plot_source <- read_csv_checked(cure_plot_source_file, "Cure plot source")
bayesian_horizon_summary <- read_csv_checked(bayesian_horizon_summary_file, "Bayesian cure horizon summary")
bayesian_plot_source <- read_csv_checked(bayesian_plot_source_file, "Bayesian cure plot source")
...
nocure_survival_data <- extract_nocure_curve_data(nocure_survival_plot, "estimated_survival_probability")
nocure_risk_data <- extract_nocure_curve_data(nocure_risk_plot, "estimated_risk_probability")
cure_survival_data <- prepare_cure_curve_data(cure_plot_source, cure_horizon_summary, "overall_survival_prob")
cure_risk_data <- prepare_cure_curve_data(cure_plot_source, cure_horizon_summary, "overall_risk_prob")
bayesian_risk_data <- prepare_bayesian_curve_data(
  bayesian_plot_source,
  bayesian_horizon_summary,
  "overall_risk_prob"
)
merged_analysis_df <- prepare_merged_analysis_dataset(source_data_file)
nocure_site_adjusted_survival <- build_nocure_site_adjusted_from_fit(nocure_fitted_models, merged_analysis_df)
nocure_site_adjusted_risk <- list(
  curve = nocure_site_adjusted_survival$curve %>%
    transmute(dataset, time_year, estimate_value = 1 - estimate_value),
  yearly = nocure_site_adjusted_survival$yearly %>%
    transmute(dataset, horizon_year, estimate_value = 1 - estimate_value)
)
...
nocure_survival_data$curve <- bind_rows(
  nocure_survival_data$curve,
  nocure_site_adjusted_survival$curve
)
nocure_survival_data$yearly <- bind_rows(
  nocure_survival_data$yearly,
  nocure_site_adjusted_survival$yearly
)
nocure_risk_data$curve <- bind_rows(
  nocure_risk_data$curve,
  nocure_site_adjusted_risk$curve
)
nocure_risk_data$yearly <- bind_rows(
  nocure_risk_data$yearly,
  nocure_site_adjusted_risk$yearly
)

survival_curve_df <- bind_rows(
  km_survival_data$curve %>% mutate(model = "KM"),
  nocure_survival_data$curve %>% mutate(model = "MLE no-cure"),
  cure_survival_data$curve %>% mutate(model = "MLE cure")
) %>%
  mutate(model = factor(model, levels = model_order))

survival_yearly_df <- bind_rows(
  km_survival_data$yearly %>% mutate(model = "KM"),
  nocure_survival_data$yearly %>% mutate(model = "MLE no-cure"),
  cure_survival_data$yearly %>% mutate(model = "MLE cure")
) %>%
  mutate(model = factor(model, levels = model_order))

risk_curve_df <- bind_rows(
  km_risk_data$curve %>% mutate(model = "KM"),
  nocure_risk_data$curve %>% mutate(model = "MLE no-cure"),
  cure_risk_data$curve %>% mutate(model = "MLE cure")
) %>%
  mutate(model = factor(model, levels = model_order))

risk_yearly_df <- bind_rows(
  km_risk_data$yearly %>% mutate(model = "KM"),
  nocure_risk_data$yearly %>% mutate(model = "MLE no-cure"),
  cure_risk_data$yearly %>% mutate(model = "MLE cure")
) %>%
  mutate(model = factor(model, levels = model_order))

risk_curve_df_with_bayesian <- bind_rows(
  risk_curve_df %>% mutate(model = as.character(model)),
  bayesian_risk_data$curve %>% mutate(model = "Bayesian cure")
) %>%
  mutate(model = factor(model, levels = model_order))

risk_yearly_df_with_bayesian <- bind_rows(
  risk_yearly_df %>% mutate(model = as.character(model)),
  bayesian_risk_data$yearly %>% mutate(model = "Bayesian cure")
) %>%
  mutate(model = factor(model, levels = model_order))

annotation_df <- prepare_followup_annotation(nocure_detail_bundle)
...
integrated_estimate_table <- bind_rows(
  ...
  risk_yearly_df_with_bayesian %>% mutate(metric = "risk")
) %>%
  mutate(
    dataset_label = unname(dataset_label_lookup[dataset]),
...
source_manifest <- tibble(
  source_name = c(
    ...
    "km_survival_plot",
    "km_risk_plot",
    "nocure_horizon_summary",
    "nocure_survival_plot",
    "nocure_risk_plot",
    "nocure_detail_bundle",
    "nocure_fitted_models",
    "source_data_file",
    "cure_horizon_summary",
    "cure_plot_source",
    "bayesian_horizon_summary",
    "bayesian_plot_source"
  ),
  path = c(
    km_dataset_summary_file,
    km_survival_plot_file,
    km_risk_plot_file,
    nocure_horizon_summary_file,
    nocure_survival_plot_file,
    nocure_risk_plot_file,
    nocure_detail_bundle_file,
    nocure_fitted_models_file,
    source_data_file,
    cure_horizon_summary_file,
    cure_plot_source_file,
    bayesian_horizon_summary_file,
    bayesian_plot_source_file
  )
)
...
plot_source_table <- build_integrated_plot_source_table(
  ...
  risk_curve_df = risk_curve_df_with_bayesian,
  risk_yearly_df = risk_yearly_df_with_bayesian,
  annotation_df = annotation_df
)
...
for (dataset_name in dataset_order) {
  ...
  risk_plot_with_bayesian <- build_combined_plot(
    dataset_name = dataset_name,
    metric = "risk",
    curve_df = risk_curve_df_with_bayesian,
    yearly_df = risk_yearly_df_with_bayesian,
    annotation_df = dataset_annotation_df,
    dataset_summary = km_dataset_summary
  )

  dataset_slug <- tolower(dataset_name)
  survival_png_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_survival_curve_with_counts__", dataset_slug, ".png"))
  survival_rds_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_survival_curve_with_counts__", dataset_slug, ".rds"))
  risk_png_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_risk_curve_with_counts__", dataset_slug, ".png"))
  risk_rds_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_risk_curve_with_counts__", dataset_slug, ".rds"))
  risk_bayesian_png_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_risk_curve_with_counts_plus_bayesian__", dataset_slug, ".png"))
  risk_bayesian_rds_file <- file.path(export_path, paste0("km_vs_mle_nocure_cure_risk_curve_with_counts_plus_bayesian__", dataset_slug, ".rds"))

  saveRDS(survival_plot, survival_rds_file)
  saveRDS(risk_plot, risk_rds_file)
  saveRDS(risk_plot_with_bayesian, risk_bayesian_rds_file)
  save_patchwork_png(survival_plot, survival_png_file)
  save_patchwork_png(risk_plot, risk_png_file)
  save_patchwork_png(risk_plot_with_bayesian, risk_bayesian_png_file)

  plot_registry[[paste0(dataset_name, "__survival")]] <- survival_plot
  plot_registry[[paste0(dataset_name, "__risk")]] <- risk_plot
  plot_registry[[paste0(dataset_name, "__risk_plus_bayesian")]] <- risk_plot_with_bayesian
  ...
  plot_manifest <- bind_rows(
    ...
    tibble(
      dataset = dataset_name,
      metric = "risk_plus_bayesian",
      png_file = risk_bayesian_png_file,
      rds_file = risk_bayesian_rds_file
    )
  )
}
...
readr::write_csv(
  plot_source_table,
  file.path(export_path, "km_vs_mle_nocure_cure_plot_source.csv")
)
readr::write_csv(
  integrated_estimate_table,
  file.path(export_path, "km_vs_mle_nocure_cure_integrated_yearly_estimates.csv")
)
readr::write_csv(
  annotation_df,
  file.path(export_path, "km_vs_mle_nocure_cure_count_annotation_source.csv")
)
readr::write_csv(
  plot_manifest,
  file.path(export_path, "km_vs_mle_nocure_cure_plot_manifest.csv")
)
readr::write_csv(
  source_manifest,
  file.path(export_path, "km_vs_mle_nocure_cure_source_manifest.csv")
)
saveRDS(
  plot_registry,
  file.path(export_path, "km_vs_mle_nocure_cure_plot_registry.rds")
)

message("Integrated KM vs MLE no-cure vs MLE cure plots saved to: ", normalizePath(export_path, winslash = "/", mustWork = FALSE))
