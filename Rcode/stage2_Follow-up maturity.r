...
export_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage2/attachments'
...
# 🔴 Define: stage-two utility functions ===============================
## 🟠 Define: file safeguards and scalar helpers ===============================
...
make_support_note <- function(support_tier_standard) {
  tier_chr <- as.character(support_tier_standard)
  
  dplyr::case_when(
    tier_chr == 'primary_supported' ~ 'Primary supported horizon',
...
normalize_horizon_registry <- function(horizon_registry) {
  ...
  raw_tier <- if ('interpretation_tier' %in% names(out)) out$interpretation_tier else rep(NA_character_, nrow(out))
  ...
  if (!'interpretation_note' %in% names(out)) {
    out <- out %>%
      mutate(
        interpretation_note = make_support_note(support_tier_standard)
      )
  }
  ...
  out %>%
    ...
    select(
      ...
      support_tier_standard, support_display_group, support_display_label,
...
## 🟠 Define: stage-one backbone readers ===============================
load_stage1_inputs <- function() {
  assert_file_exists('stage1_analysis_datasets', stage1_analysis_datasets_file)
  assert_file_exists('stage1_backbone_bundle', stage1_backbone_bundle_file)
  assert_file_exists('stage1_scaling_registry', stage1_scaling_registry_file)
  assert_file_exists('stage1_horizon_registry', stage1_horizon_registry_file)
  assert_file_exists('stage1_threshold_registry', stage1_threshold_registry_file)
  assert_file_exists('stage1_metadata_registry', stage1_metadata_registry_file)
  assert_file_exists('stage1_dataset_registry', stage1_dataset_registry_file)
  
  list(
    analysis_datasets = readRDS(stage1_analysis_datasets_file),
    backbone_bundle = readRDS(stage1_backbone_bundle_file),
    scaling_registry = read_csv_required(stage1_scaling_registry_file),
    horizon_registry = normalize_horizon_registry(read_csv_required(stage1_horizon_registry_file)),
    threshold_registry = read_csv_required(stage1_threshold_registry_file),
    metadata_registry = read_csv_required(stage1_metadata_registry_file),
    dataset_registry = read_csv_required(stage1_dataset_registry_file),
    export_manifest = if (file.exists(stage1_export_manifest_file)) read_csv_required(stage1_export_manifest_file) else NULL
  )
}

validate_stage1_backbone <- function(stage1_inputs) {
  required_dataset_names <- dataset_order
  observed_dataset_names <- names(stage1_inputs$analysis_datasets)
  
  if (!setequal(required_dataset_names, observed_dataset_names)) {
    stop(
      sprintf(
        'Stage 1 analysis datasets must contain exactly PNU, SNU, and merged. Found: %s',
  ...
  observed_horizon_datasets <- sort(unique(as.character(stage1_inputs$horizon_registry$dataset)))
  if (!setequal(required_dataset_names, observed_horizon_datasets)) {
    stop(
      sprintf(
        'Stage 1 horizon registry must contain exactly PNU, SNU, and merged. Found: %s',
        paste(observed_horizon_datasets, collapse = ', ')
      ),
      call. = FALSE
    )
  }
  
  horizon_check <- stage1_inputs$horizon_registry %>%
    group_by(dataset) %>%
    summarise(
      horizon_vector = list(sort(unique(as.integer(horizon_year)))),
      .groups = 'drop'
    )
  
  bad_horizon_datasets <- horizon_check$dataset[
    !vapply(horizon_check$horizon_vector, function(x) identical(x, 1:10), logical(1))
  ]
  
  if (length(bad_horizon_datasets) > 0L) {
    stop(
      sprintf(
        'Each dataset in the Stage 1 horizon registry must contain exactly 1:10 years. Problem datasets: %s',
        paste(bad_horizon_datasets, collapse = ', ')
      ),
      call. = FALSE
    )
  }
...
## 🟠 Define: support-tier mapping helpers ===============================
...
## 🟠 Define: subgroup registry builders ===============================
build_group_registry <- function(analysis_datasets) {
  ...
  bind_rows(group_rows) %>%
    mutate(
      dataset = factor(dataset, levels = dataset_order) %>% as.character()
    )
}
...
## 🟠 Define: survival-curve functions ===============================
...
## 🟠 Define: horizon-summary functions ===============================
compute_horizon_row <- function(
  ...
  instability_marker <- dplyr::case_when(
    ...
    risk_set_n < 5L ~ 'very_sparse',
    risk_set_n < 10L ~ 'sparse',
    risk_set_n / n_total < 0.10 ~ 'low_fraction',
    TRUE ~ 'stable_enough'
  )
...
## 🟠 Define: composition and risk-table functions ===============================
...
compute_numbers_at_risk <- function(df, dataset_name, subgroup_kind, subgroup_value, group_id, panel_label, risk_table_times_year, horizon_registry_dataset) {
  ...
  tibble(
    ...
    mutate(
      ...
      interpretation_tier = if_else(
        time_year == 0,
        'baseline',
        dplyr::coalesce(matched_tier, 'unregistered')
      ),
      support_tier_standard = if_else(
        time_year == 0,
        'baseline',
        dplyr::coalesce(matched_standard, 'unregistered')
      ),
      support_display_group = make_support_display_group(
        support_tier_standard = support_tier_standard,
...
## 🟠 Define: merged site-contribution diagnostics ===============================
  ...
  bind_rows(lapply(seq_len(nrow(structure_registry)), function(s) {
    ...
    bind_rows(lapply(seq_len(nrow(horizon_registry_dataset)), function(i) {
      ...
      site_counts <- df %>%
        ...
        group_by(site) %>%
        summarise(
          site_eligible_n = sum(is_eligible, na.rm = TRUE),
          .groups = 'drop'
        ) %>%
        right_join(tibble(site = site_levels), by = 'site') %>%
        mutate(
          site_eligible_n = dplyr::coalesce(as.integer(site_eligible_n), 0L)
        ) %>%
        arrange(site)
...
## 🟠 Define: reshaping and metadata functions ===============================
...
make_stage2_metadata <- function(
  ...
  tibble::tribble(
    ...
    'stage', 'patch_level', 'patched_user_specified_paths_preferred_site_adjusted_reporting_v7',
    'inputs', 'data_path_resolved', data_path,
    'inputs', 'data_path_configuration_rule', 'Use the user-specified data_path directly; do not auto-resolve from project root or legacy paths.',
    ...
    'summaries', 'horizon_summary_scope_note', 'stage2_followup_horizon_summary.csv contains horizon-level metrics plus merged site-contribution descriptors, late-horizon site-dominance warning fields, plot-masking flags, and explicit preferred_for_reporting fields.',
    ...
    'validation', 'raw_reconstruction_note', 'Validation now includes direct raw-data reconstructions of group sizes, numbers at risk, merged site administrative end dates, and preferred-reporting flags in addition to internal cross-output consistency checks.',
    'validation', 'sex_plot_exported', if (isTRUE(export_sex_panel_plots)) 'TRUE' else 'FALSE',
    'outputs', 'export_path_resolved', export_path,
    'outputs', 'export_path_configuration_rule', 'Use the user-specified export_path directly; do not auto-resolve from project root or legacy paths.',
    'outputs', 'threshold_vector_loaded_from_stage1', paste(format(threshold_vector, trim = TRUE, scientific = FALSE), collapse = ',')
  )
}

# 🔴 Define: validation audit functions ===============================
## 🟠 Define: output-internal consistency audits ===============================
...
audit_merged_site_consistency <- function(horizon_summary, reverse_km_summary) {
  ...
  detail_horizon <- bind_rows(lapply(site_map, function(site_name) {
    ...
    if (nrow(single_df) != nrow(merged_df) || nrow(single_df) == 0L) {
      return(
        tibble(
          ...
          compare_column = 'row_alignment',
          max_abs_diff = NA_real_,
          issue_type = 'row_mismatch'
        )
      )
    }
    ...
    tibble(
      ...
      max_abs_diff = map_dbl(compare_cols_horizon, function(col_name) {
        safe_max_abs_diff(single_df[[col_name]], merged_df[[col_name]])
      }),
  ...
  detail_reverse <- bind_rows(lapply(site_map, function(site_name) {
    ...
    tibble(
      ...
      max_abs_diff = map_dbl(compare_cols_reverse, function(col_name) {
        safe_max_abs_diff(single_rev[[col_name]], merged_rev[[col_name]])
      }),
      issue_type = 'value_difference'
    ) %>%
      filter(is.na(max_abs_diff) | max_abs_diff > 1e-8)
  ...
  summary <- bind_rows(
    tibble(
      ...
      check_name = 'merged_site_consistency_horizon',
      status = if (nrow(detail_horizon) == 0L) 'pass' else 'fail',
      n_problem_rows = nrow(detail_horizon),
      check_note = 'Merged site_adjusted site-specific horizon summaries must match the corresponding single-site overall summaries.'
    ),
    tibble(
      validation_group = 'cross_dataset_consistency',
      check_name = 'merged_site_consistency_reverse_km',
      status = if (nrow(detail_reverse) == 0L) 'pass' else 'fail',
      n_problem_rows = nrow(detail_reverse),
      check_note = 'Merged site-specific reverse-KM summaries must match the corresponding single-site overall reverse-KM summaries.'
    )
  )
...
audit_site_contribution_alignment <- function(horizon_summary, site_contribution_summary) {
  target_rows <- horizon_summary %>%
    filter(
      dataset == 'merged',
      subgroup_kind %in% c('overall', 'sex'),
      analysis_structure %in% c('site_free', 'site_adjusted')
    ) %>%
    distinct(
      dataset, subgroup_kind, subgroup_value, group_id, panel_label,
      analysis_structure, horizon_year, eligible_n
    )
  ...
  detail <- target_rows %>%
    left_join(
      site_contribution_summary %>%
        select(
          dataset, subgroup_kind, subgroup_value, group_id, panel_label,
          analysis_structure, horizon_year, total_eligible_n
        ),
      by = c(
        'dataset', 'subgroup_kind', 'subgroup_value', 'group_id', 'panel_label',
        'analysis_structure', 'horizon_year'
    ...
    mutate(
      eligible_difference = abs(as.numeric(eligible_n) - as.numeric(total_eligible_n))
    ) %>%
    filter(
      is.na(total_eligible_n) |
        is.na(eligible_n) |
        eligible_difference > 1e-8
    ) %>%
...
audit_site_support_label_alignment <- function(numbers_at_risk_data, horizon_summary, site_support_dataset_lookup) {
  ...
  detail <- bind_rows(lapply(seq_len(nrow(site_support_dataset_lookup)), function(i) {
    ...
    bind_rows(
      compare_support_label_frames(
        reference_df = reference_horizon,
        comparison_df = merged_horizon,
        key_col = 'horizon_year',
      ...
      compare_support_label_frames(
        reference_df = reference_risk,
        comparison_df = merged_risk,
        key_col = 'time_year',
        compare_cols = compare_cols,
        site_value = site_value,
        support_dataset = support_dataset,
        compare_block = 'numbers_at_risk'
      )
    )
  })) %>%
...
audit_site_dominance_warning_gate <- function(horizon_summary) {
  eligible_rows <- horizon_summary %>%
    filter(
      dataset == 'merged',
      subgroup_kind %in% c('overall', 'sex'),
      analysis_structure %in% c('site_free', 'site_adjusted')
    ...
    distinct(
      ...
      support_tier_standard, site_dominance_status,
      site_dominance_warning_flag
    )
  
  expected_flag <- should_raise_site_dominance_warning(
    site_dominance_status = eligible_rows$site_dominance_status,
    support_tier_standard = eligible_rows$support_tier_standard,
    horizon_year = eligible_rows$horizon_year
  )
  
  detail <- eligible_rows %>%
    mutate(expected_site_dominance_warning_flag = as.logical(expected_flag)) %>%
    filter(site_dominance_warning_flag != expected_site_dominance_warning_flag) %>%
    mutate(check_name = 'site_dominance_warning_gate')
  
  summary <- tibble(
    validation_group = 'internal_consistency',
    check_name = 'site_dominance_warning_gate',
    status = if (nrow(detail) == 0L) 'pass' else 'fail',
    n_problem_rows = nrow(detail),
    check_note = sprintf('Merged site-dominance warnings must follow the late-horizon gate: single_site_only always warns; single_site_dominant warns only from year %d onward and, when configured, outside primary-supported horizons.', site_dominance_warning_min_horizon_year)
  )
...
audit_reporting_preference_rule <- function(horizon_summary) {
  detail <- horizon_summary %>%
    ...
    mutate(
      ...
      expected_reporting_priority = dplyr::case_when(
        expected_preferred_for_reporting ~ 1L,
        TRUE ~ 2L
      )
    ) %>%
    filter(
      preferred_for_reporting != expected_preferred_for_reporting |
        reporting_priority != expected_reporting_priority
    ) %>%
...
## 🟠 Define: raw-reconstruction audits ===============================
...
audit_numbers_at_risk_reconstruction_from_raw <- function(analysis_datasets_stage2, group_registry, numbers_at_risk_data, risk_table_times_year) {
  tol <- 1e-10
  
  expected_n_risk <- bind_rows(lapply(seq_len(nrow(group_registry)), function(i) {
    group_row <- group_registry[i, ]
    raw_df <- filter_group_data(
      df = analysis_datasets_stage2[[group_row$dataset]],
      subgroup_kind = group_row$subgroup_kind,
      subgroup_value = group_row$subgroup_value
    )
    
    tibble(
      dataset = group_row$dataset,
      subgroup_kind = group_row$subgroup_kind,
      subgroup_value = group_row$subgroup_value,
      group_id = group_row$group_id,
      panel_label = group_row$panel_label,
      time_year = as.numeric(risk_table_times_year),
      expected_n_risk = vapply(risk_table_times_year, function(t_year) {
        sum(raw_df$time_year >= (as.numeric(t_year) - tol), na.rm = TRUE)
      }, integer(1))
    )
  }))
  
  detail <- expected_n_risk %>%
    left_join(
      numbers_at_risk_data %>%
        select(dataset, subgroup_kind, subgroup_value, group_id, panel_label, time_year, n_risk),
      by = c('dataset', 'subgroup_kind', 'subgroup_value', 'group_id', 'panel_label', 'time_year')
    ) %>%
    mutate(
      issue_type = case_when(
        is.na(n_risk) ~ 'missing_numbers_at_risk_row',
        abs(as.numeric(n_risk) - as.numeric(expected_n_risk)) > 1e-8 ~ 'value_mismatch',
        TRUE ~ 'match'
      )
    ) %>%
    filter(issue_type != 'match') %>%
    mutate(check_name = 'numbers_at_risk_reconstruction_from_raw')
  
  summary <- tibble(
    validation_group = 'raw_reconstruction',
    check_name = 'numbers_at_risk_reconstruction_from_raw',
    status = if (nrow(detail) == 0L) 'pass' else 'fail',
    n_problem_rows = nrow(detail),
    check_note = 'Numbers-at-risk values at 0,1,...,10 years must be reproducible directly from the raw Stage 2 group data.'
  )
...
run_stage2_validation_audits <- function(
  ...
  audit_results <- list(
    ...
    audit_site_dominance_warning_gate(horizon_summary),
    ...
    audit_numbers_at_risk_reconstruction_from_raw(analysis_datasets_stage2, group_registry, numbers_at_risk_data, risk_table_times_year),
...
# 🔴 Define: quality-flag builders ===============================
## 🟠 Define: sparse-tail and caution registries ===============================
make_quality_flags <- function(horizon_summary, reverse_km_summary) {
  ...
  sparse_flags <- horizon_summary %>%
    ...
    transmute(
      ...
      flag_severity = case_when(
        completeness_plot_status == 'very_sparse' ~ 'warning',
        TRUE ~ 'caution'
      ),
      flag_code = paste0('completeness_', completeness_plot_status),
      dataset, subgroup_kind, subgroup_value, group_id, panel_label,
  ...
  dominance_flags <- horizon_summary %>%
    ...
    distinct(
      dataset, subgroup_kind, subgroup_value, group_id, panel_label,
      analysis_structure, horizon_year,
      dominant_site, dominant_site_fraction,
      site_dominance_status, site_dominance_warning_note
    ) %>%
    transmute(
      flag_domain = 'merged_site_contribution',
      flag_severity = case_when(
        site_dominance_status == 'single_site_only' ~ 'warning',
        TRUE ~ 'caution'
      ),
  ...
  reverse_flags <- reverse_km_summary %>%
    filter(
      !is.na(reverse_km_median_followup_year) &
        (is.na(reverse_km_median_followup_lcl_year) | is.na(reverse_km_median_followup_ucl_year))
    ) %>%
    transmute(
...
confirm_stage2_exports <- function(export_manifest, export_path) {
  expected_paths <- file.path(export_path, export_manifest$file_name)
  missing_paths <- expected_paths[!file.exists(expected_paths)]
  
  if (length(missing_paths) > 0L) {
    stop(
      sprintf(
        'Stage 2 completed, but some expected export files are missing: %s',
...
## 🟠 Define: stage-two plot themes and exporters ===============================
...
save_stage2_plots <- function(
  ...
  completeness_plot <- completeness_plot_subset %>%
    ...
    geom_point(
      data = completeness_plot_data_stable,
      size = 1.7,
      na.rm = TRUE
    ) +
    geom_point(
      data = completeness_plot_data_sparse,
      size = 2.0,
      shape = 1,
      stroke = 0.8,
      na.rm = TRUE
    ...
    geom_point(
      data = completeness_plot_data_very_sparse,
      size = 2.1,
      shape = 4,
      stroke = 0.9,
      na.rm = TRUE
  ...
  pdf(output_pdf_path, width = 14, height = 10, onefile = TRUE)
...
stage1_inputs <- load_stage1_inputs()
validate_stage1_backbone(stage1_inputs)

analysis_datasets <- stage1_inputs$analysis_datasets[dataset_order]
horizon_registry <- stage1_inputs$horizon_registry
threshold_registry <- stage1_inputs$threshold_registry
threshold_vector <- extract_threshold_vector(threshold_registry)
scaling_registry_stage1 <- stage1_inputs$scaling_registry
dataset_registry <- stage1_inputs$dataset_registry

# 🔴 Prepare: stage-two datasets and administrative limits ===============================
analysis_datasets_prepped <- list(
  PNU = prepare_stage2_dataset(analysis_datasets[['PNU']], 'PNU', site_mode = 'single'),
  SNU = prepare_stage2_dataset(analysis_datasets[['SNU']], 'SNU', site_mode = 'single'),
  merged = prepare_stage2_dataset(analysis_datasets[['merged']], 'merged', site_mode = 'merged')
)
...
site_admin_lookup <- compute_site_admin_lookup(analysis_datasets_prepped_df[['merged']])
merged_admin_end_date <- max(analysis_datasets_prepped_df[['merged']]$followup_date, na.rm = TRUE)

analysis_datasets_stage2 <- list(
  PNU = attach_admin_dates(analysis_datasets_prepped_df[['PNU']], 'PNU', site_admin_lookup, merged_admin_end_date),
  SNU = attach_admin_dates(analysis_datasets_prepped_df[['SNU']], 'SNU', site_admin_lookup, merged_admin_end_date),
  merged = attach_admin_dates(analysis_datasets_prepped_df[['merged']], 'merged', site_admin_lookup, merged_admin_end_date)
)
...
# 🔴 Compute: follow-up maturity objects ===============================
...
# 🔴 Combine: consolidated stage-two tables ===============================
...
curve_data_numbers <- numbers_at_risk_data %>%
  ...
  select(
    ...
    interpretation_tier, support_tier_standard, support_display_group, support_display_label,
    n_total, n_risk_fraction
  )
...
stage2_metadata_registry <- make_stage2_metadata(
  ...
  merged_admin_end_date = merged_admin_end_date,
...
# 🔴 Check: internal consistency and planned-stage fit ===============================
...
if (nrow(validation_failures) > 0L) {
  stop(
    paste(
      'Stage 2 validation failed for the following checks:',
      paste(validation_failures$check_name, collapse = ', ')
    ),
    call. = FALSE
  )
}
...
# 🔴 Export: patched stage-two outputs ===============================
...
export_manifest <- tibble(
  ...
  description = c(
    ...
    'Horizon-specific follow-up maturity table with support-tier fields, late-horizon site-dominance warning fields, plot-masking flags, and preferred_for_reporting fields that prioritize merged site_adjusted completeness',
    'Long-format horizon-level follow-up maturity table without repeated group-level reverse-KM fields',
    'Transition-only and remission-sensitive horizon summaries saved side by side without repeated group-level reverse-KM fields and with preferred-reporting markers',
    'Curve-level source-of-truth data for KM, reverse KM, numbers-at-risk, and event/censoring/remission composition plots',
    'Explicit source-of-truth data frame for the completeness plot, derived from the horizon summary and carrying preferred-reporting markers',
    ...
    'Non-fatal sparse-tail, masking, reverse-KM CI, and gated merged site-dominance caution flags',
    'Reusable patched Stage 2 follow-up maturity bundle',
    'Stage 2 follow-up maturity plots for overall and site panels',
    'Manifest of all Stage 2 exported files',
    'Plain-text run log reporting the user-specified input/output paths and export confirmation'
  ),
...
run_log_lines <- c(
  ...
  sprintf('[Stage 2] Exported files    : %d', nrow(export_manifest)),
  paste(sprintf(' - %s', export_manifest$file_name), collapse = '\n')
)
writeLines(run_log_lines, con = file.path(export_path, 'stage2_run_log.txt'))

confirm_stage2_exports(export_manifest = export_manifest, export_path = export_path)
message(sprintf('[Stage 2] Saved %d files to: %s', nrow(export_manifest), export_path))