...
data_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage1/attachments'  # folder containing Stage 1 exported CSV/RDS artifacts
export_path <- '/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage3/attachments'  # all Stage 3 outputs will be written here
...
subgroup_variables <- c("sex_label")  # used only when run_subgroup_km = TRUE
subgroup_min_n <- 30L
...
validate_stage1_backbone <- function(
  ...
  required_dataset_cols <- c(
    ...
    "status_num", "event_main", "censor_main", "remission_flag"
  )
  ...
  if (nrow(dataset_registry) == 0 || nrow(horizon_registry) == 0 || nrow(threshold_registry) == 0 ||
      nrow(metadata_registry) == 0 || nrow(scaling_registry) == 0) {
    stop("One or more required Stage 1 registry/metadata/scaling files are empty.", call. = FALSE)
  }
...
extract_survfit_lookup <- function(fit, times, strata_variable = NULL) {
  fit_summary <- summary(fit, times = times, extend = TRUE)
  
  if (is.null(fit_summary$strata)) {
    out <- tibble(
      assignment_value = "overall",
      horizon_year = as.integer(round(as.numeric(fit_summary$time))),
    ...
    out <- tibble(
      ...
      select(assignment_value, horizon_year, km_survival)
  }
...
build_subject_predictions <- function(df, horizons_tbl, risk_lookup, meta) {
  ...
  subject_base <- df %>%
    transmute(
      ...
      status_num,
      event_main,
      assignment_value = if (identical(assignment_variable, "__overall__")) {
        "overall"
      } else {
  ...
  out <- tidyr::crossing(
    ...
    left_join(
      risk_lookup %>%
        distinct(assignment_value, horizon_year, km_survival, km_risk),
      by = c("assignment_value", "horizon_year")
    ) %>%
    mutate(
      dataset = meta$dataset,
      model_class = "KM_benchmark",
      ...
      subgroup_kind = meta$subgroup_kind,
      subgroup_variable = null_to_na_character(meta$subgroup_variable),
      subgroup_value_assigned = assignment_value,
      risk_assignment_rule = meta$risk_assignment_rule,
      horizon_support_label = interpretation_tier,
  ...
  if (anyNA(out$km_survival) || anyNA(out$km_risk)) {
    stop(
      sprintf("[%s - %s] Missing KM survival/risk values after benchmark assignment.",
              meta$dataset, meta$benchmark_id),
      call. = FALSE
    )
  }
...
make_overall_benchmark <- function(df, dataset_name, horizons_tbl) {
  ...
  risk_lookup <- extract_survfit_lookup(
    fit = fit,
    times = horizons_tbl$horizon_year,
  ...
  meta <- list(
    ...
    benchmark_label = "Overall KM benchmark",
    benchmark_scope = "primary",
    site_branch = "site_free",
    subgroup_kind = "overall",
    subgroup_variable = NULL,
    assignment_variable = "__overall__",
    risk_assignment_rule = "overall cohort-level KM risk applied to all subjects in the dataset"
  )
...
make_site_adjusted_benchmark <- function(df, dataset_name, horizons_tbl) {
  fit <- survival::survfit(
...
make_optional_subgroup_benchmark <- function(df, dataset_name, horizons_tbl, subgroup_variable, subgroup_min_n_value) {
...
make_classification_table <- function(subject_predictions, threshold_registry) {
  subject_predictions %>%
    ...
    mutate(
      predicted_positive = km_risk >= threshold,
...
make_stage3_metadata <- function(
  ...
  tibble(
    ...
    metadata_name = c(
      ...
      "primary_benchmark_rule", "merged_site_structure_rule", "benchmark_output_source_of_truth",
      "classification_output_file",
      "run_subgroup_km", "subgroup_variables", "subgroup_min_n",
      "stage1_created_at", "stage1_common_rule_file"
    ),
    metadata_value = c(
      "Stage 3 KM benchmark classifier",
      "Operationalize overall KM as the primary benchmark classifier and retain merged site-adjusted structure.",
      "TRUE_KM_only",
      ...
      "overall KM is the primary benchmark classifier",
      "merged retains both site-free overall KM and site-adjusted site-specific KM assignment",
      prediction_file_name,
      classification_file_name,
...
benchmark_fit_names <- purrr::map_chr(
  ...
  ~ paste0(.x$registry$dataset, "__", .x$registry$benchmark_id)
)
...
stage3_export_manifest <- tibble(
  ...
  description = c(
    "Registry of Stage 3 KM benchmark variants",
    ...
    "Long-format KM benchmark classification table aligned to Stage 1 horizons and thresholds",
    "Stage 3 metadata and evaluation-rule notes",
