...
resolve_shared_path <- function(candidates, label) {
  candidates <- unique(trimws(as.character(candidates)))
  candidates <- candidates[!is.na(candidates) & nzchar(candidates)]
  
  existing_idx <- which(file.exists(candidates) | dir.exists(candidates))
  if (length(existing_idx) > 0L) {
    return(normalizePath(candidates[[existing_idx[[1]]]], winslash = "/", mustWork = FALSE))
  }
  
  parent_idx <- which(vapply(dirname(candidates), dir.exists, logical(1)))
  if (length(parent_idx) > 0L) {
    return(normalizePath(candidates[[parent_idx[[1]]]], winslash = "/", mustWork = FALSE))
  }
  
  stop(
    sprintf("%s could not be resolved from candidates: %s", label, paste(candidates, collapse = ", ")),
    call. = FALSE
  )
}

stage6_run_root_candidates <- if (.Platform$OS.type == "windows") {
  c(
    "C:/Users/clair/Dropbox/Data Analysis/STAGE6",
    "/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/STAGE6"
  )
} else {
  c(
    "/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/STAGE6",
...
stage1_attachment_candidates <- if (.Platform$OS.type == "windows") {
  c(
    "C:/Users/clair/Dropbox/Data Analysis/🟦Stage1/attachments",
    "/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/🟦Stage1/attachments"
  )
} else {
  c(
    "/Users/ido/Library/CloudStorage/Dropbox/Data Analysis/🟦Stage1/attachments",
...
run_root_dir <- resolve_shared_path(stage6_run_root_candidates, "run_root_dir")
...
canonical_export_path <- run_root_dir
...
# •	refresh_run_state <- TRUE
# = 남아 있는 작업 흔적을 강제로 다시 지워라
# •	refresh_run_state <- FALSE
# = 남아 있으면 재사용하고, 없으면 새로 만들어라
...
# Local lock controls (Windows + Dropbox hotfix)
# Put lock directories outside Dropbox / synced folders.
lock_root_dir <- NA_character_

# Wait long enough for the first worker to finish the observed-only setup.
worker_init_lock_timeout_sec <- 3600L

# Runtime status logging is auxiliary. Never let a runtime-status lock failure kill a shard worker.
runtime_status_lock_timeout_sec <- 2L
runtime_status_skip_on_lock_failure <- TRUE

# Windows sometimes delays directory deletion on synced folders.
lock_release_retries <- 60L
lock_release_wait_sec <- 0.10
...
# Input paths
data_path <- resolve_shared_path(stage1_attachment_candidates, "data_path")
...
if (identical(run_mode, "smoke")) {
  ...
  bootstrap_reps_xie <- 999L
  bootstrap_reps_hsu <- 999L
  optim_maxit <- 5000L
  optim_reltol <- 1e-10
...
resolve_lock_root_dir <- function(run_root_dir, lock_root_dir = NA_character_) {
  user_lock_root <- trimws(as.character(lock_root_dir)[1])
  
  if (!is.na(user_lock_root) && nzchar(user_lock_root)) {
    return(normalizePath(user_lock_root, winslash = "/", mustWork = FALSE))
  }
  
  run_root_id <- gsub(
    pattern = "[^A-Za-z0-9_.-]+",
    replacement = "_",
    x = normalizePath(run_root_dir, winslash = "/", mustWork = FALSE)
  )
  
  if (.Platform$OS.type == "windows") {
    parent_dir <- Sys.getenv("LOCALAPPDATA", unset = "")
    if (!nzchar(parent_dir)) {
      parent_dir <- tempdir(check = FALSE)
    }
  } else {
    parent_dir <- file.path(path.expand("~"), ".stage6_local_locks")
  }
  
  normalizePath(
    file.path(parent_dir, "stage6_lockspace", run_root_id),
    winslash = "/",
    mustWork = FALSE
  )
}

local_lock_root_dir <- resolve_lock_root_dir(run_root_dir, lock_root_dir = lock_root_dir)
...
release_dir_lock <- function(lock_dir, retries = lock_release_retries, wait_sec = lock_release_wait_sec) {
  if (!dir.exists(lock_dir)) {
    return(TRUE)
  }
  
  retries <- max(1L, as.integer(retries))
  wait_sec <- max(0, as.numeric(wait_sec))
  
  for (ii in seq_len(retries)) {
    suppressWarnings(unlink(lock_dir, recursive = TRUE, force = TRUE))
    if (!dir.exists(lock_dir)) {
      return(TRUE)
    }
    if (wait_sec > 0) {
...
break_stale_lock_if_needed <- function(lock_dir, stale_after_sec = lock_stale_after_sec) {
  ...
  isTRUE(release_dir_lock(lock_dir))
}

with_dir_lock <- function(lock_dir, expr, timeout_sec = 600, poll_sec = 1, stale_after_sec = max(lock_stale_after_sec, timeout_sec + 60L), fail_on_release = FALSE) {
  ...
  on.exit({
    if (!lock_released) {
      ok <- release_dir_lock(lock_dir)
      if (!ok && isTRUE(fail_on_release)) {
        warning(sprintf("Failed to release lock: %s", lock_dir), call. = FALSE)
      }
    }
  }, add = TRUE)
  ...
  tryCatch(
    {
      ...
      }, timeout_sec = runtime_status_lock_timeout_sec, poll_sec = 0.10, fail_on_release = FALSE)
      
      invisible(TRUE)
    },
    error = function(e) {
      log_step("Runtime status update skipped: ", conditionMessage(e), level = "WARN")
      
      if (!isTRUE(runtime_status_skip_on_lock_failure)) {
        stop(e)
      }
...
reset_run_state_files <- function() {
  ...
  invisible(lapply(c(init_lock_dir, registry_lock_dir, reduce_lock_dir, runtime_lock_dir), function(lock_path) {
...
build_stage6_run_manifest <- function() {
  tibble::tibble(
    item = c(
      ...
      "locks_dir", "lock_root_dir", "worker_init_lock_timeout_sec",
      "runtime_status_lock_timeout_sec", "runtime_status_skip_on_lock_failure",
      "lock_release_retries", "lock_release_wait_sec",
      "bootstrap_completion_policy"
    ),
    value = c(
      run_root_dir, export_path, canonical_export_path, run_mode,
      ...
      locks_dir, as.character(lock_root_dir), as.character(worker_init_lock_timeout_sec),
      as.character(runtime_status_lock_timeout_sec),
      as.character(runtime_status_skip_on_lock_failure),
      as.character(lock_release_retries),
      as.character(lock_release_wait_sec),
      "valid_bootstrap_target_with_auto_topup_shards"
    )
  )
}
...
make_registry_row <- function(task_type, dataset_key, shard_id, rep_ids, created_now, working_family = NA_character_, note = NA_character_, n_shards = shard_id) {
  if (identical(task_type, "xie")) {
    shard_file <- make_xie_shard_file(dataset_key, shard_id, n_shards)
    observed_file <- make_xie_observed_file(dataset_key)
    task_id <- sprintf("xie__%s__%03d", dataset_key, shard_id)
  } else if (identical(task_type, "hsu")) {
    shard_file <- make_hsu_shard_file(dataset_key, working_family, shard_id, n_shards)
    observed_file <- make_hsu_observed_file(dataset_key, working_family)
    task_id <- sprintf("hsu__%s__%s__%03d", dataset_key, working_family, shard_id)
  } else {
    stop(sprintf("Unsupported task_type for registry row construction: %s", task_type), call. = FALSE)
  }
  ...
  tibble::tibble(
    ...
    working_family = if (identical(task_type, "xie")) NA_character_ else as.character(working_family),
    shard_id = as.character(shard_id),
    n_shards = as.character(n_shards),
...
build_bootstrap_group_targets <- function() {
  xie_targets <- tibble::tibble(
    task_type = "xie",
    dataset_key = c("PNU", "SNU", "merged__site_free"),
    working_family = NA_character_,
    target_valid_reps = as.integer(bootstrap_reps_xie),
    shard_size = as.integer(xie_shard_size)
  )
  
  hsu_targets <- expand.grid(
    dataset_key = dataset_order_full,
    working_family = candidate_hsu_families,
    stringsAsFactors = FALSE
  ) %>%
    tibble::as_tibble() %>%
    dplyr::mutate(
      task_type = "hsu",
      target_valid_reps = as.integer(bootstrap_reps_hsu),
      shard_size = as.integer(hsu_shard_size)
    ) %>%
    dplyr::select(task_type, dataset_key, working_family, target_valid_reps, shard_size)
  
  dplyr::bind_rows(xie_targets, hsu_targets)
}

filter_shard_group <- function(registry_df, task_type, dataset_key, working_family = NA_character_) {
...
collect_completed_bootstrap_stats_long <- function(registry_df, task_type, dataset_key, working_family = NA_character_) {
  ...
  if (nrow(subset_df) == 0L) {
    return(tibble::tibble(rep_id = integer(), statistic = numeric()))
  }
...
build_shard_registry <- function() {
  ...
  target_df <- build_bootstrap_group_targets()
  
  for (ii in seq_len(nrow(target_df))) {
    target_row <- target_df[ii, , drop = FALSE]
    rep_ranges <- split_rep_ranges(target_row$target_valid_reps[[1]], target_row$shard_size[[1]])
    if (length(rep_ranges) == 0L) {
      next
    }
    
    for (shard_idx in seq_along(rep_ranges)) {
      registry_rows[[length(registry_rows) + 1L]] <- make_registry_row(
        task_type = target_row$task_type[[1]],
        dataset_key = target_row$dataset_key[[1]],
        working_family = target_row$working_family[[1]],
        shard_id = shard_idx,
        rep_ids = rep_ranges[[shard_idx]],
        created_now = created_now,
        n_shards = length(rep_ranges),
        note = if (identical(target_row$task_type[[1]], "xie")) "Xie bootstrap shard" else "HSU bootstrap shard"
      )
    }
  }
...
collect_bootstrap_stats_from_registry <- function(registry_df, task_type, dataset_key, total_reps = NULL, working_family = NA_character_) {
  long_df <- collect_completed_bootstrap_stats_long(
    registry_df = registry_df,
    task_type = task_type,
    dataset_key = dataset_key,
    working_family = working_family
  ...
  if (is.null(total_reps)) {
    return(as.numeric(long_df$statistic))
  }
  ...
  valid <- which(long_df$rep_id >= 1L & long_df$rep_id <= total_reps)
  if (length(valid) > 0L) {
    out[long_df$rep_id[valid]] <- long_df$statistic[valid]
  }
  
  out
}

build_topup_shards_for_group <- function(registry_df, task_type, dataset_key, target_valid_reps, shard_size, working_family = NA_character_) {
  group_df <- filter_shard_group(
    registry_df = registry_df,
    task_type = task_type,
    dataset_key = dataset_key,
    working_family = working_family
  ...
  if (nrow(group_df) == 0L || any(group_df$status %in% c("pending", "running", "failed"))) {
    return(tibble::tibble())
  }
  
  completed_stats <- collect_completed_bootstrap_stats_long(
    registry_df = registry_df,
    task_type = task_type,
    dataset_key = dataset_key,
    working_family = working_family
  )
  current_valid_reps <- sum(is.finite(completed_stats$statistic))
  deficit <- as.integer(target_valid_reps) - current_valid_reps
  
  if (!is.finite(deficit) || deficit <= 0L) {
    return(tibble::tibble())
  }
  
  existing_shard_ids <- as_integer_or_na(group_df$shard_id)
  existing_shard_ids <- existing_shard_ids[is.finite(existing_shard_ids)]
  next_shard_id <- if (length(existing_shard_ids) > 0L) max(existing_shard_ids) + 1L else 1L
  
  existing_rep_ends <- as_integer_or_na(group_df$rep_id_end)
  existing_rep_ends <- existing_rep_ends[is.finite(existing_rep_ends)]
  next_rep_id <- if (length(existing_rep_ends) > 0L) max(existing_rep_ends) + 1L else 1L
  
  rep_ranges <- split_rep_ranges(deficit, shard_size)
  if (length(rep_ranges) == 0L) {
    return(tibble::tibble())
  }
  ...
  created_now <- now_utc_chr()
  out_rows <- list()
  for (ii in seq_along(rep_ranges)) {
    rep_ids <- next_rep_id + rep_ranges[[ii]] - 1L
    shard_id <- next_shard_id + ii - 1L
    out_rows[[length(out_rows) + 1L]] <- make_registry_row(
      task_type = task_type,
      dataset_key = dataset_key,
      working_family = working_family,
      shard_id = shard_id,
      ...
      n_shards = shard_id,
      note = paste0(
        "Auto-generated top-up shard because only ",
        current_valid_reps,
        "/",
        target_valid_reps,
        " valid bootstrap statistics were available."
      )
    )
  }
...
append_topup_shards_if_needed <- function(registry_df) {
  ...
  for (ii in seq_len(nrow(target_df))) {
    target_row <- target_df[ii, , drop = FALSE]
    new_rows <- build_topup_shards_for_group(
      registry_df = registry_df,
      task_type = target_row$task_type[[1]],
      dataset_key = target_row$dataset_key[[1]],
      working_family = target_row$working_family[[1]],
      target_valid_reps = target_row$target_valid_reps[[1]],
      shard_size = target_row$shard_size[[1]]
    )
    
    if (nrow(new_rows) > 0L) {
      added_rows[[length(added_rows) + 1L]] <- new_rows
      registry_df <- dplyr::bind_rows(registry_df, new_rows) %>%
        normalize_shard_registry() %>%
        dplyr::arrange(task_type, match(dataset_key, dataset_order_full), working_family, as_integer_or_na(shard_id))
    }
  }
  
  added_rows <- compact_bind_rows(added_rows)
  list(
    registry_df = registry_df,
    added_rows = added_rows,
    added_any = nrow(added_rows) > 0L
  )
}
...
prepare_observed_and_registry <- function() {
  ...
  state_bundle <- list(
    ...
    config = list(
      ...
      locks_dir = locks_dir,
      lock_root_dir = lock_root_dir,
      worker_init_lock_timeout_sec = worker_init_lock_timeout_sec,
      runtime_status_lock_timeout_sec = runtime_status_lock_timeout_sec,
      runtime_status_skip_on_lock_failure = runtime_status_skip_on_lock_failure,
      lock_release_retries = lock_release_retries,
      lock_release_wait_sec = lock_release_wait_sec
    ),
    inputs = list(
      formula_registry = ingest$formula_registry,
...
state_is_ready <- function() {
  !isTRUE(refresh_run_state) &&
    file.exists(state_initialized_file) &&
    file.exists(state_bundle_file) &&
    file.exists(shard_registry_file)
}
...
initialize_if_needed <- function() {
    ...
    tryCatch(
      {
        with_dir_lock(registry_lock_dir, {
          registry_df <- requeue_stale_and_retryable_tasks(read_shard_registry())
          write_shard_registry(registry_df)
        }, timeout_sec = max(5L, min(worker_claim_lock_timeout_sec, 30L)), poll_sec = 0.25, fail_on_release = FALSE)
      },
      error = function(e) {
        log_step("Existing initialized state detected; registry refresh skipped: ", conditionMessage(e), level = "WARN")
      }
    )
  ...
  with_dir_lock(init_lock_dir, {
    if (isTRUE(refresh_run_state) || !state_is_ready()) {
      prepare_observed_and_registry()
    } else {
      registry_df <- requeue_stale_and_retryable_tasks(read_shard_registry())
      write_shard_registry(registry_df)
    }
  }, timeout_sec = worker_init_lock_timeout_sec, poll_sec = 0.5, fail_on_release = FALSE)
...
reduce_and_export_stage6 <- function() {
  ...
  registry_state <- with_dir_lock(registry_lock_dir, {
    registry_df <- read_shard_registry()
    
    if (any(registry_df$status %in% c("pending", "running"))) {
      return(list(
        ready_to_reduce = FALSE,
        registry_df = registry_df,
        topup_added = FALSE,
        added_rows = tibble::tibble()
      ))
    }
    ...
    topup_result <- append_topup_shards_if_needed(registry_df)
    if (isTRUE(topup_result$added_any)) {
      write_shard_registry(topup_result$registry_df)
      return(list(
        ready_to_reduce = FALSE,
        registry_df = topup_result$registry_df,
        topup_added = TRUE,
        added_rows = topup_result$added_rows,
  ...
  }, timeout_sec = worker_reduce_lock_timeout_sec, poll_sec = 0.25)
  
  if (isTRUE(registry_state$blocked_by_failed)) {
    log_step("Reduction blocked because failed shard(s) remain in the registry.", level = "WARN")
    return(FALSE)
  }
  
  if (isTRUE(registry_state$topup_added)) {
    added_rows <- registry_state$added_rows
    added_groups <- if (nrow(added_rows) > 0L) {
      added_rows %>%
        dplyr::mutate(
          working_family = dplyr::if_else(is.na(working_family) | !nzchar(working_family), "none", working_family)
        ) %>%
        dplyr::count(task_type, dataset_key, working_family, name = "n_new_shards")
    } else {
      tibble::tibble()
    }
    
    log_step(
      "Valid bootstrap counts were below target. Appended ",
      nrow(added_rows),
      " top-up shard(s) and deferred reduction."
    )
    
    if (nrow(added_groups) > 0L) {
      for (ii in seq_len(nrow(added_groups))) {
        log_step(
          "Top-up queued: task_type=",
          added_groups$task_type[[ii]],
          ", dataset_key=",
          added_groups$dataset_key[[ii]],
          ", working_family=",
          added_groups$working_family[[ii]],
          ", n_new_shards=",
          added_groups$n_new_shards[[ii]]
        )
      }
    }
    
    write_runtime_status(
      "bootstrap_topup_enqueued",
      note = paste("Appended", nrow(added_rows), "top-up shard(s) because valid bootstrap counts were below target.")
    )
    return(FALSE)
  }
  
  if (!isTRUE(registry_state$ready_to_reduce)) {
    return(FALSE)
  }
  
  registry_df <- registry_state$registry_df
  
  log_step("All shards complete and valid bootstrap targets met. Starting Stage 6 reducer.")
  write_runtime_status("reducing", note = "All shards complete and valid bootstrap targets met; Stage 6 reducer started.")
