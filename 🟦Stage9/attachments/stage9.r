# rm(list = ls())
# Fixed items:
# 1) benchmark delta plots separate comparison_id so merged overall and site-adjusted series are never connected
# 2) plot panels use explicit panel labels to remove empty facet combinations across datasets/formulas
# 3) stage9 metadata records the full exported file inventory
# 4) remission/event-free component availability flags are carried into classification, risk-summary, and delta outputs
# 5) component-availability flags are now model-structure based rather than inferred from NA/non-NA values
# 6) cause-specific transition-only fallback is labeled explicitly and no longer masquerades as a full remission decomposition
# 7) projection/reporting guardrails are exported in tables and projection horizons are visually de-emphasized in plots
# 8) export-manifest construction now takes explicit arguments rather than depending on globals

# 🔴 Configure: paths and stage-9 controls ===============================
stage1_path <- "/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage1/attachments"
export_path <- "/Volumes/ObsidianVault/Obsidian/☔️Papers_Writing(논문 쓰기)/📙Currently working/⬛조현병 베이지안 생존분석/🟧1.데이터 분석 New/🟦Stage9/attachments"
...
projection_plot_caption <- "Grey shading marks projection horizons; prioritize rows with supported_for_reporting = TRUE for primary interpretation."

options(stringsAsFactors = FALSE, scipen = 999)

# 🔴 Initialize: package namespace and runtime logging ===============================
...
if (length(missing_packages) > 0L) {
  stop(
    sprintf(
      "Install required packages before running Stage 9: %s",
...
# 🔴 Define: utility helpers and validators ===============================
...
derive_horizon_support_label <- function(interpretation_tier, primary_supported_flag) {
  tier_chr <- trimws(as.character(interpretation_tier))
  normalized_tier <- tolower(gsub("[^a-z0-9]+", "_", tier_chr))
  primary_flag <- !is.na(primary_supported_flag) & as.logical(primary_supported_flag)
  dplyr::case_when(
    primary_flag ~ "primary_supported",
    normalized_tier %in% c("primary_supported", "primarysupported", "primary") ~ "primary_supported",
    normalized_tier %in% c("secondary", "secondary_supported", "secondarysupported") ~ "secondary",
    normalized_tier %in% c("sensitivity", "sensitive", "limited_support", "limited_supported", "intermediate") ~ "sensitivity",
    normalized_tier %in% c("projection", "projected", "tail", "tail_projection") ~ "projection",
    TRUE ~ ifelse(primary_flag, "primary_supported", "secondary")
  )
}
...
augment_horizon_registry_fields <- function(horizon_registry) {
  horizon_registry %>%
    mutate(
      horizon_support_label = dplyr::if_else(
        is.na(horizon_support_label) | !nzchar(trimws(as.character(horizon_support_label))),
        derive_horizon_support_label(
          interpretation_tier = interpretation_tier,
          primary_supported_flag = primary_supported_flag
        ),
        as.character(horizon_support_label)
      ),
      supported_for_reporting = as.logical(horizon_support_label != "projection"),
      projection_flag = as.logical(horizon_support_label == "projection"),
      reporting_status = dplyr::if_else(projection_flag, "projection_only", "supported")
    )
}
...
validate_component_annotations <- function(tbl, table_label) {
  ...
  remission_bad <- !equal_or_both_na(tbl$remission_component_available, expected$remission_component_available)
  eventfree_bad <- !equal_or_both_na(tbl$eventfree_component_available, expected$eventfree_component_available)
  mode_bad <- !equal_or_both_na(tbl$component_mode, expected$component_mode)
  note_bad <- !equal_or_both_na(tbl$component_note, expected$component_note)
  
  if (any(remission_bad, na.rm = TRUE)) {
    stop(sprintf("[%s] remission_component_available does not match model structure.", table_label), call. = FALSE)
  }
  if (any(eventfree_bad, na.rm = TRUE)) {
    stop(sprintf("[%s] eventfree_component_available does not match model structure.", table_label), call. = FALSE)
  }
  if (any(mode_bad, na.rm = TRUE)) {
    stop(sprintf("[%s] component_mode does not match model structure.", table_label), call. = FALSE)
  }
  if (any(note_bad, na.rm = TRUE)) {
...
validate_horizon_reporting_fields <- function(tbl, table_label) {
  ...
  required_cols <- c("horizon_support_label", "supported_for_reporting", "projection_flag", "reporting_status")
  assert_required_columns(tbl, required_cols, paste0("validate_horizon_reporting_fields:", table_label))
  
  bad_projection <- tbl$projection_flag & tbl$supported_for_reporting
  if (any(bad_projection, na.rm = TRUE)) {
    stop(sprintf("[%s] projection_flag cannot be TRUE when supported_for_reporting is TRUE.", table_label), call. = FALSE)
  }
  
  bad_supported <- (!tbl$projection_flag) & (!tbl$supported_for_reporting)
  if (any(bad_supported, na.rm = TRUE)) {
    stop(sprintf("[%s] only projection horizons may have supported_for_reporting == FALSE.", table_label), call. = FALSE)
  }
  
  bad_status <- (!tbl$projection_flag & tbl$reporting_status != "supported") |
    (tbl$projection_flag & tbl$reporting_status != "projection_only")
  if (any(bad_status, na.rm = TRUE)) {
    stop(sprintf("[%s] reporting_status does not match projection_flag.", table_label), call. = FALSE)
  }
...
build_projection_rectangles <- function(df, panel_col = "panel_label") {
  if (nrow(df) == 0L || !all(c(panel_col, "horizon_year", "projection_flag") %in% names(df))) {
    return(tibble())
  }
  
  df %>%
    mutate(projection_flag = as.logical(projection_flag)) %>%
    group_by(across(all_of(panel_col))) %>%
    summarise(
      has_projection = any(projection_flag, na.rm = TRUE),
      xmin = {
        if (any(projection_flag, na.rm = TRUE)) {
          min(horizon_year[projection_flag], na.rm = TRUE) - 0.5
        } else {
          NA_real_
        }
      },
...
## 🟠 Define: output integrity checks ===============================
...
assert_prob_in_unit_interval <- function(x, label, tol = 1e-8) {
  ...
  if (length(bad) > 0L) {
    stop(
      sprintf("%s has %d values outside [0, 1] tolerance.", label, length(bad)),
      call. = FALSE
    )
  }
  invisible(TRUE)
...
assert_prob_sum_close_to_one <- function(x, label, tol = 1e-6) {
  ...
  if (length(bad) > 0L) {
    stop(
      sprintf("%s has %d values not summing to 1 within tolerance.", label, length(bad)),
      call. = FALSE
    )
  }
  invisible(TRUE)
...
validate_subject_predictions <- function(predictions_long, block_name) {
  ...
  required_cols <- c(
    ...
    "prediction_level", "model_class", "model_family",
  ...
  if (nrow(main_rows) > 0L) {
    ...
    if (any(main_rows$remission_component_available, na.rm = TRUE)) {
      stop(sprintf("[%s] Main mirror rows must keep remission_component_available == FALSE.", block_name), call. = FALSE)
    }
  }
  
  sensitivity_rows <- predictions_long %>% filter(analysis_variant == "remission_sensitive")
  if (nrow(sensitivity_rows) > 0L && block_name %in% c("benchmark", "cause_specific")) {
    full_rows <- sensitivity_rows %>% filter(remission_component_available & eventfree_component_available)
    if (nrow(full_rows) > 0L) {
      if (anyNA(full_rows$predicted_remission_risk) || anyNA(full_rows$predicted_eventfree_prob)) {
        stop(
          sprintf("[%s] Full remission-sensitive rows must have non-missing remission/event-free probabilities.", block_name),
          call. = FALSE
        )
      }
      assert_prob_sum_close_to_one(
        full_rows$predicted_transition_risk +
          full_rows$predicted_remission_risk +
          full_rows$predicted_eventfree_prob,
        paste0(block_name, " remission-sensitive probability sum")
      )
    }
  }
  
  if (nrow(sensitivity_rows) > 0L && identical(block_name, "subdistribution")) {
    if (!all(is.na(sensitivity_rows$predicted_remission_risk))) {
      assert_prob_in_unit_interval(
        sensitivity_rows$predicted_remission_risk,
        paste0(block_name, " sensitivity predicted_remission_risk")
      )
    }
    if (!all(is.na(sensitivity_rows$predicted_eventfree_prob))) {
      assert_prob_in_unit_interval(
        sensitivity_rows$predicted_eventfree_prob,
        paste0(block_name, " sensitivity predicted_eventfree_prob")
      )
    }
  }
...
validate_classification_table <- function(classification_tbl, table_label) {
  ...
  required_cols <- c(
    ...
    "false_positive_rate", "false_positive_burden", "false_positive_burden_known_nonevent",
    "specificity", "ppv", "tpr",
  ...
  assert_prob_in_unit_interval(classification_tbl$positive_classification_rate, paste0(table_label, " positive_classification_rate"))
  assert_prob_in_unit_interval(classification_tbl$positive_classification_rate_evaluable, paste0(table_label, " positive_classification_rate_evaluable"))
  assert_prob_in_unit_interval(classification_tbl$false_positive_rate, paste0(table_label, " false_positive_rate"))
  assert_prob_in_unit_interval(classification_tbl$false_positive_burden, paste0(table_label, " false_positive_burden"))
  assert_prob_in_unit_interval(classification_tbl$false_positive_burden_known_nonevent, paste0(table_label, " false_positive_burden_known_nonevent"))
  assert_prob_in_unit_interval(classification_tbl$specificity, paste0(table_label, " specificity"))
  assert_prob_in_unit_interval(classification_tbl$ppv, paste0(table_label, " ppv"))
  assert_prob_in_unit_interval(classification_tbl$tpr, paste0(table_label, " tpr"))
  assert_prob_in_unit_interval(classification_tbl$event_prevalence_all, paste0(table_label, " event_prevalence_all"))
  assert_prob_in_unit_interval(classification_tbl$event_prevalence_evaluable, paste0(table_label, " event_prevalence_evaluable"))
  assert_prob_in_unit_interval(classification_tbl$remission_prevalence_all, paste0(table_label, " remission_prevalence_all"))
  
  positive_classification_rate_evaluable_check <- safe_divide(
    classification_tbl$n_predicted_positive_evaluable,
    classification_tbl$n_evaluable_by_horizon
  )
  evaluable_mismatch <- abs(classification_tbl$positive_classification_rate_evaluable - positive_classification_rate_evaluable_check)
  evaluable_mismatch[is.na(evaluable_mismatch)] <- 0
  if (any(evaluable_mismatch > 1e-10)) {
    stop(sprintf("[%s] positive_classification_rate_evaluable does not match evaluable numerator/denominator.", table_label), call. = FALSE)
  }
...
validate_delta_table <- function(delta_tbl, table_label) {
  ...
  required_cols <- c(
    ...
    "main_positive_classification_rate", "sensitivity_positive_classification_rate",
    "main_positive_classification_rate_evaluable", "sensitivity_positive_classification_rate_evaluable",
    "main_false_positive_rate", "sensitivity_false_positive_rate",
    "main_net_benefit", "sensitivity_net_benefit",
    "main_net_benefit_evaluable", "sensitivity_net_benefit_evaluable",
    "main_standardized_net_benefit", "sensitivity_standardized_net_benefit",
    "horizon_support_label", "supported_for_reporting", "projection_flag", "reporting_status",
    "main_remission_component_available", "sensitivity_remission_component_available",
    "main_eventfree_component_available", "sensitivity_eventfree_component_available",
    "main_component_mode", "sensitivity_component_mode",
    "main_component_note", "sensitivity_component_note"
  )
  ...
  expected_main <- derive_component_annotations(
    comparison_scope = delta_tbl$comparison_scope,
    comparison_family = delta_tbl$comparison_family,
    analysis_variant = rep("main_censoring_mirror", nrow(delta_tbl)),
    sensitivity_method = delta_tbl$sensitivity_method
  )
  expected_sensitivity <- derive_component_annotations(
    comparison_scope = delta_tbl$comparison_scope,
    comparison_family = delta_tbl$comparison_family,
    analysis_variant = rep("remission_sensitive", nrow(delta_tbl)),
    sensitivity_method = delta_tbl$sensitivity_method
  )
  
  if (any(!equal_or_both_na(delta_tbl$main_remission_component_available, expected_main$remission_component_available), na.rm = TRUE)) {
    stop(sprintf("[%s] main_remission_component_available mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$main_eventfree_component_available, expected_main$eventfree_component_available), na.rm = TRUE)) {
    stop(sprintf("[%s] main_eventfree_component_available mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$main_component_mode, expected_main$component_mode), na.rm = TRUE)) {
    stop(sprintf("[%s] main_component_mode mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$main_component_note, expected_main$component_note), na.rm = TRUE)) {
    stop(sprintf("[%s] main_component_note mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$sensitivity_remission_component_available, expected_sensitivity$remission_component_available), na.rm = TRUE)) {
    stop(sprintf("[%s] sensitivity_remission_component_available mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$sensitivity_eventfree_component_available, expected_sensitivity$eventfree_component_available), na.rm = TRUE)) {
    stop(sprintf("[%s] sensitivity_eventfree_component_available mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$sensitivity_component_mode, expected_sensitivity$component_mode), na.rm = TRUE)) {
    stop(sprintf("[%s] sensitivity_component_mode mismatch.", table_label), call. = FALSE)
  }
  if (any(!equal_or_both_na(delta_tbl$sensitivity_component_note, expected_sensitivity$component_note), na.rm = TRUE)) {
    stop(sprintf("[%s] sensitivity_component_note mismatch.", table_label), call. = FALSE)
  }
  
  delta_checks <- list(
    risk_change_signed = delta_tbl$sensitivity_mean_predicted_transition_risk - delta_tbl$main_mean_predicted_transition_risk,
    remission_risk_added = delta_tbl$sensitivity_mean_predicted_remission_risk - delta_tbl$main_mean_predicted_remission_risk,
    false_positive_burden_change = delta_tbl$sensitivity_false_positive_burden - delta_tbl$main_false_positive_burden,
    false_positive_per_100_change = delta_tbl$sensitivity_false_positive_per_100 - delta_tbl$main_false_positive_per_100,
    positive_classification_rate_change = delta_tbl$sensitivity_positive_classification_rate - delta_tbl$main_positive_classification_rate,
    positive_classification_rate_evaluable_change = delta_tbl$sensitivity_positive_classification_rate_evaluable - delta_tbl$main_positive_classification_rate_evaluable,
    false_positive_rate_change = delta_tbl$sensitivity_false_positive_rate - delta_tbl$main_false_positive_rate,
    net_benefit_change = delta_tbl$sensitivity_net_benefit - delta_tbl$main_net_benefit,
    net_benefit_evaluable_change = delta_tbl$sensitivity_net_benefit_evaluable - delta_tbl$main_net_benefit_evaluable,
    standardized_net_benefit_change = delta_tbl$sensitivity_standardized_net_benefit - delta_tbl$main_standardized_net_benefit
  )
  
  for (check_name in names(delta_checks)) {
    reference <- delta_checks[[check_name]]
    observed <- delta_tbl[[check_name]]
    diff_val <- abs(observed - reference)
    diff_val[is.na(diff_val)] <- 0
    if (any(diff_val > 1e-10)) {
      stop(sprintf("[%s] %s does not match its component columns.", table_label, check_name), call. = FALSE)
    }
  }
...
validate_export_manifest <- function(export_manifest, export_path) {
  ...
  if (length(missing_files) > 0L) {
    stop(
      sprintf(
        "Export manifest references files that were not created: %s",
...
## 🟠 Define: factor and panel helpers ===============================
...
validate_stage1_backbone <- function(
  ...
  if (!all(required_dataset_names %in% observed_dataset_names)) {
    stop(
      sprintf(
        "Stage 1 analysis datasets must include PNU, SNU, merged. Observed: %s",
        paste(observed_dataset_names, collapse = ", ")
      ),
      call. = FALSE
    )
  }
  ...
  required_dataset_cols <- c(
    "unique_person_id", "id", "site", "sex_num", "sex_label",
    "age_exact_entry", "age_s", "days_followup", "time_year",
    "status_num", "status_label", "event_main", "remission_flag", "censor_main"
  )
  ...
  for (dataset_name in required_dataset_names) {
    ...
    if (length(missing_cols) > 0L) {
      stop(
        sprintf(
          "[%s] Stage 1 analysis dataset is missing required columns: %s",
          dataset_name,
          paste(missing_cols, collapse = ", ")
        ),
        call. = FALSE
  ...
  required_formula_cols <- c(
    ...
    "formula_full", "uses_site", "uses_age_sex_interaction",
...
## 🟠 Define: stage metadata builders ===============================
build_stage9_metadata <- function(
  ...
  core_rows <- bind_rows(
    ...
    tibble(
      ...
      metadata_name = c("main_event_definition", "main_censoring_definition", "remission_sensitive_rule"),
      metadata_value = c(
        "status_num == 1",
        "status_num %in% c(0, 2)",
    ...
    tibble(
      ...
      metadata_value = c(
        ...
        "supported_for_reporting = FALSE only when horizon_support_label == projection; projection horizons are shaded in the Stage 9 plot PDF"
      )
    ),
    tibble(
  ...
  bind_rows(
    ...
    if (isTRUE(make_plot_pdf)) {
      tibble(
        ...
        metadata_value = "stage9_plots.pdf"
      )
    } else {
      NULL
    },
...
build_stage9_export_manifest <- function(make_plot_pdf, save_fit_objects) {
  bind_rows(
    tibble(
      file_name = c(
        "stage9_status_summary.csv",
    ...
    if (isTRUE(make_plot_pdf)) {
      tibble(
        file_name = "stage9_plots.pdf",
        object_name = "stage9_plot_pdf",
        role = "plots from exported data frames with projection shading"
      )
    } else {
      NULL
    },
...
## 🟠 Define: status and grouping helpers ===============================
...
# 🔴 Define: nonparametric transition and remission engines ===============================
## 🟠 Define: Kaplan-Meier mirror builders ===============================
...
## 🟠 Define: Aalen-Johansen builders ===============================
...
## 🟠 Define: benchmark horizon constructors ===============================
build_group_horizon_table <- function(df_group, dataset_name, meta_row, horizons_tbl) {
  ...
  horizons_tbl %>%
    select(
      ...
      interpretation_note, horizon_support_label,
...
build_benchmark_outputs <- function(df, dataset_name, horizons_tbl, allow_site_adjusted = TRUE) {
  meta_tbl <- make_benchmark_meta(
    dataset_name = dataset_name,
    df = df,
    allow_site_adjusted = allow_site_adjusted
  )
  ...
  for (i in seq_along(comparison_ids)) {
    ...
    expected_group_rows <- nrow(meta_rows) * nrow(horizons_tbl)
    if (nrow(group_horizon_tbl) != expected_group_rows) {
      stop(
        sprintf(
          "[benchmark:%s:%s] Group horizon table row count mismatch. Expected %d, observed %d.",
          dataset_name,
          comparison_id,
          expected_group_rows,
          nrow(group_horizon_tbl)
        ),
        call. = FALSE
      )
    }
      ...
      left_join(
        group_horizon_tbl %>%
          select(
            ...
            model_class, main_model_family, sensitivity_model_family,
    ...
    expected_subject_rows <- nrow(subject_base) * nrow(horizons_tbl)
    if (nrow(subject_joined) != expected_subject_rows) {
      stop(
        sprintf(
          "[benchmark:%s:%s] Subject-horizon join row count mismatch. Expected %d, observed %d.",
          dataset_name,
          comparison_id,
...
## 🟠 Define: outcome framework mirrors ===============================
...
## 🟠 Define: prediction summaries and change tables ===============================
make_risk_summary_from_predictions <- function(predictions_long) {
  ...
  if (nrow(joined) != nrow(main_tbl) || nrow(joined) != nrow(sensitivity_tbl)) {
    stop(
      "Main/sensitivity prediction join mismatch in `make_risk_summary_from_predictions()`.",
      call. = FALSE
    )
  }
...
# 🔴 Define: regression and prediction engines ===============================
## 🟠 Define: formula and design-matrix utilities ===============================
...
## 🟠 Define: Fine-Gray optional fitters ===============================
...
## 🟠 Define: subject-prediction assemblers ===============================
make_prediction_long <- function(
  ...
  if (nrow(out) != expected_rows) {
    stop(
      sprintf(
        "[%s:%s] Subject prediction row count mismatch. Expected %d, observed %d.",
        dataset_name,
        formula_row$formula_id[[1]],
        expected_rows,
        nrow(out)
      ),
      call. = FALSE
...
run_cause_specific_block_for_dataset <- function(df, dataset_name, formula_registry, horizons_tbl, threshold_registry) {
  ...
  for (i in seq_len(nrow(formula_rows))) {
    ...
    log_step(
      "Cause-specific mirror fitting: dataset=", dataset_name,
      ", formula_id=", formula_row$formula_id[[1]]
    )
...
run_subdistribution_block_for_dataset <- function(df, dataset_name, formula_registry, horizons_tbl, threshold_registry) {
  ...
  for (i in seq_len(nrow(formula_rows))) {
    ...
    log_step(
      "Fine-Gray fitting: dataset=", dataset_name,
      ", formula_id=", formula_row$formula_id[[1]]
    )
...
analysis_datasets <- read_stage1_rds(stage1_datasets_file)
...
if (!"interpretation_tier" %in% names(horizon_registry)) {
  stop("Stage 1 horizon registry must contain `interpretation_tier`.", call. = FALSE)
}

if (!"primary_supported_flag" %in% names(horizon_registry)) {
  horizon_registry$primary_supported_flag <- FALSE
}

if (!"interpretation_note" %in% names(horizon_registry)) {
  horizon_registry$interpretation_note <- NA_character_
}

if (!"horizon_support_label" %in% names(horizon_registry)) {
  horizon_registry$horizon_support_label <- NA_character_
}
...
# 🔴 Fit: nonparametric remission-sensitive benchmarks ===============================
...
# 🔴 Fit: cause-specific and subdistribution mirrors ===============================
...
# 🔴 Export: stage-9 source-of-truth tables ===============================
## 🟠 Save: core CSV files ===============================
...
# 🔴 Plot: shared data-frame driven PDF summaries ===============================
## 🟠 Draw: remission sensitivity panels ===============================
if (isTRUE(make_plot_pdf)) {
  plot_file <- file.path(export_path, "stage9_plots.pdf")
  grDevices::pdf(plot_file, width = 11, height = 8.5)
  plot_device_open <- TRUE
  on.exit(
    {
      if (exists("plot_device_open", inherits = FALSE) && isTRUE(plot_device_open)) {
        grDevices::dev.off()
      }
    },
    add = TRUE
  ...
  if (nrow(benchmark_delta_vs_main) > 0L) {
    ...
    print(
      ggplot(benchmark_delta_plot_data, aes(x = horizon_year, y = false_positive_burden_change, group = 1)) +
        {
    ...
    print(
      ggplot(benchmark_delta_plot_data, aes(x = horizon_year, y = net_benefit_change, group = 1)) +
        {
          if (nrow(benchmark_delta_projection_rect) > 0L) {
            geom_rect(
              data = benchmark_delta_projection_rect,
  ...
  if (nrow(cause_specific_risk_summary) > 0L) {
    ...
    cause_plot_data <- cause_specific_risk_summary %>%
      select(
        dataset, formula_label, horizon_year, projection_flag,
    ...
    print(
      ggplot(cause_plot_data, aes(x = horizon_year, y = value, group = curve_type, linetype = curve_type)) +
        {
  ...
  if (nrow(cause_specific_delta_vs_main) > 0L) {
    ...
    print(
      ggplot(cause_delta_plot_data, aes(x = horizon_year, y = false_positive_burden_change, group = 1)) +
        {
    ...
    print(
      ggplot(cause_delta_plot_data, aes(x = horizon_year, y = net_benefit_change, group = 1)) +
        {
          if (nrow(cause_delta_projection_rect) > 0L) {
            geom_rect(
              data = cause_delta_projection_rect,
  ...
  if (nrow(subdistribution_risk_summary) > 0L) {
    ...
    print(
      ggplot(fg_plot_data, aes(x = horizon_year, y = value, group = curve_type, linetype = curve_type)) +
        {
  ...
  if (nrow(subdistribution_delta_vs_main) > 0L) {
    ...
    print(
      ggplot(subdistribution_delta_plot_data, aes(x = horizon_year, y = false_positive_burden_change, group = 1)) +
        {
    ...
    print(
      ggplot(subdistribution_delta_plot_data, aes(x = horizon_year, y = net_benefit_change, group = 1)) +
        {
          if (nrow(subdistribution_delta_projection_rect) > 0L) {
            geom_rect(
              data = subdistribution_delta_projection_rect,
  ...
  grDevices::dev.off()
  plot_device_open <- FALSE
}

# 🔴 Build: final metadata bundles and persisted objects ===============================
## 🟠 Compose: manifest and fit-object bundle ===============================
...
## 🟠 Compose: metadata and full output bundle ===============================
...
stage9_output_bundle <- list(
  ...
  config = list(
    ...
    cox_ties = cox_ties,
    projection_guardrail_enabled = TRUE
  ),
  stage1_inputs = list(
  ...
  outputs = list(
    ...
    stage9_plot_pdf = if (isTRUE(make_plot_pdf)) "stage9_plots.pdf" else NULL,
