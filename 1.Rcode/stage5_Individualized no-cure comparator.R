...
calibration_fit_attempt_reason_pattern <- paste(
  c(
    "offset_model_not_converged",
    "slope_model_not_converged",
    "offset_boundary",
    "slope_boundary"
  ),
  collapse = "|"
...
stage_spec_version <- "stage5_noncure_revised_v8"
...
flag_is_one <- function(x) {
  x_int <- suppressWarnings(as.integer(x)[1])
  isTRUE(!is.na(x_int) && x_int == 1L)
}
...
derive_calibration_failure_reason <- function(calibration_stats, horizon_support_info = NULL) {
  ...
  support_flag <- flag_is_one(calibration_stats$calibration_support_flag %||% 0L)
  fit_success_flag <- flag_is_one(calibration_stats$calibration_fit_success_flag %||% 0L)
  binary_support_flag <- flag_is_one(
    support_info$binary_outcome_support_flag %||%
      calibration_stats$binary_outcome_support_flag %||%
      0L
  )
  ...
  if (!support_flag) {
    ...
    if (flag_is_one(support_info$beyond_last_followup_flag %||% 0L)) {
      reasons <- c(reasons, "projection_beyond_last_followup")
    }
    
    if (!flag_is_one(calibration_stats$calibration_minimum_n_flag %||% 1L)) {
    ...
    return(collapse_nonempty_chr(reasons))
  }
  
  if (!flag_is_one(calibration_stats$calibration_offset_converged_flag %||% 1L)) {
    reasons <- c(reasons, "offset_model_not_converged")
  }
  
  if (!flag_is_one(calibration_stats$calibration_slope_converged_flag %||% 1L)) {
    reasons <- c(reasons, "slope_model_not_converged")
  }
  
  if (flag_is_one(calibration_stats$calibration_offset_boundary_flag %||% 0L)) {
    reasons <- c(reasons, "offset_boundary")
  }
  
  if (flag_is_one(calibration_stats$calibration_slope_boundary_flag %||% 0L)) {
...
refresh_calibration_failure_reason_column <- function(df) {
  out <- tibble::as_tibble(df)
  ...
  required_cols <- c(
    ...
    "calibration_minimum_n_flag",
    "calibration_offset_converged_flag",
    "calibration_slope_converged_flag",
    "calibration_offset_boundary_flag",
    "calibration_slope_boundary_flag",
    "binary_outcome_support_flag",
    "beyond_last_followup_flag"
  )
  ...
  out$calibration_failure_reason <- purrr::pmap_chr(
    out[, required_cols, drop = FALSE],
    function(...) {
      row_vals <- list(...)
      derive_calibration_failure_reason(
        calibration_stats = row_vals,
        horizon_support_info = row_vals
      )
    }
  )
...
finalize_calibration_diagnostics_long <- function(calibration_diagnostics_long, formula_registry_core, horizon_registry) {
  calibration_diagnostics_long %>%
    ...
    join_horizon_metadata(horizon_registry) %>%
    collapse_dataset_key_suffixes() %>%
    refresh_calibration_failure_reason_column() %>%
...
build_stage5_qc_summary <- function(
  ...
  calibration_failure_reason_chr <- if (nrow(calibration_diagnostics_long) > 0 && "calibration_failure_reason" %in% names(calibration_diagnostics_long)) {
    ifelse(
      is.na(calibration_diagnostics_long$calibration_failure_reason),
      "",
      as.character(calibration_diagnostics_long$calibration_failure_reason)
    )
  } else {
    rep("", nrow(calibration_diagnostics_long))
  }
  
  unsupported_calibration_fit_attempt_reason_count <- if (nrow(calibration_diagnostics_long) > 0) {
    sum(
      calibration_diagnostics_long$calibration_support_flag == 0L &
        grepl(
          pattern = calibration_fit_attempt_reason_pattern,
          x = calibration_failure_reason_chr
        ),
      na.rm = TRUE
    )
  } else {
  ...
  tibble(
    ...
    unsupported_calibration_fit_attempt_reason_count = as.integer(unsupported_calibration_fit_attempt_reason_count),
...
validate_stage5_outputs <- function(qc_summary) {
  ...
  if (qc$unsupported_calibration_fit_attempt_reason_count > 0) {
    stop("Stage 5 calibration failure reasons incorrectly report fit-attempt failures for unsupported horizons.", call. = FALSE)
  }
...
if (reuse_existing_core_candidate) {
  ...
  if (reuse_qc_ok) {
    core_build_mode <- "reused_existing_core_refreshed_metadata"
  }
}
...
stage5_results_bundle <- list(
  ...
  calibration_target = calibration_target_label,
  mean_calibration_reference = mean_calibration_reference_label,
  calibration_failure_reason_rule = "fit_attempt_failures_reported_only_when_calibration_support_flag_equals_1",
  calibration_failure_reason_fit_attempt_pattern = calibration_fit_attempt_reason_pattern,
  common_horizons_year = common_horizons_year,
  prediction_horizons_for_plots = prediction_horizons_for_plots,
  risk_thresholds = risk_thresholds,
...
export_manifest <- tibble(
  ...
  description = c(
    ...
    "Model-by-horizon calibration fit diagnostics including support, projection status, convergence, boundary, suppression flags, and support-aware failure reasons.",
