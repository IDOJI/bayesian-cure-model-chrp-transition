...
fit_calibration_offset_model <- function(y, lp, w) {
  ...
  if (inherits(fit_obj$fit, "error")) {
    return(list(
      ...
      converged = FALSE,
      boundary = FALSE,
      message = safe_char(conditionMessage(fit_obj$fit))
    ))
  }
...
if (
  ...
  if (validate_existing_subject_horizon_risk_long(existing_risk_long, model_plan, common_horizons_year)) {
    core_reused_flag <- TRUE
    model_registry <- readr::read_csv(model_registry_file, show_col_types = FALSE)
    subject_horizon_risk_long <- existing_risk_long
    model_performance_long <- readr::read_csv(performance_long_file, show_col_types = FALSE)
    calibration_bins_long <- readr::read_csv(calibration_bins_file, show_col_types = FALSE)
    calibration_diagnostics_long <- readr::read_csv(calibration_diagnostics_file, show_col_types = FALSE)
...
export_manifest <- tibble(
  ...
  description = c(
    ...
    "Model-by-horizon calibration diagnostics with support gating and failure reasons",
