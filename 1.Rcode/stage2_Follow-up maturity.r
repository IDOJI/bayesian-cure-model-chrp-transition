# source/stage2.r
# Patch changes:
# 1) stage2_followup_curve_data.csv now excludes numbers_at_risk rows.
# 2) Reused bundle/CSV outputs are upgraded to the current curve-data contract.
# 3) Raw merged file provenance is recorded as "not_used" when date recovery is unnecessary.
...
current_patch_level <- 'stage2_followup_curve_contract_and_provenance_v10'
...
compatible_reuse_patch_levels <- c(
  ...
  'stage2_followup_curve_contract_and_provenance_v10'
)
...
make_stage2_metadata <- function(
  ...
  tibble::tribble(
    ...
    'plots', 'numbers_at_risk_source_of_truth', 'stage2_numbers_at_risk.csv is the exclusive canonical source-of-truth for the numbers-at-risk tile plot.',
...
export_manifest <- tibble(
  ...
  description = c(
    ...
    'Curve-level source-of-truth data for KM, reverse-KM, and event/censoring/remission composition plots only; numbers-at-risk is intentionally excluded and sourced exclusively from stage2_numbers_at_risk.csv',
...
run_log_lines <- c(
  ...
  sprintf('[Stage 2] Raw merged file   : %s', raw_merged_file_used_label),
