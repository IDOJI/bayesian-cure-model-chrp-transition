...
export_path         <- stage8a_export_path
...
find_stage_file <- function(stage_path, pattern, recursive = TRUE) {
  if (!dir.exists(stage_path)) {
    stop("Directory does not exist: ", stage_path, call. = FALSE)
  }
  ...
  if (length(hits) == 0L) {
    stop("No matching file found in: ", stage_path, " | pattern: ", pattern, call. = FALSE)
  }
  
  if (length(hits) > 1L) {
    warning("Multiple matching files found. Using the first one: ", hits[1], call. = FALSE)
  }
  
  hits[1]
}

# 🟧 Locate required files from stage folders ===============================
...
stage8_metadata_registry <- bind_rows(
  ...
  metadata_tbl(
    ...
    c(
      normalize_existing_path(project_root),
      ...
      normalize_existing_path(stage8a_export_path),
