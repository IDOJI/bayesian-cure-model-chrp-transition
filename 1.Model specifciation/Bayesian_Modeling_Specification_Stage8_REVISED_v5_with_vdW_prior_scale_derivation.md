...
**Integrated clarification in this copy:** the derivation of the stored prior scale vector $s_0^{vdW}$ from van der Werf Table 2 adjusted-IRR 95% confidence intervals is now written out explicitly, including the coefficient-to-table mapping, the 30–39 anchor used for the current `age >=30` slot, and the zero-covariance reconstruction rule that reproduces the interaction-term prior scales exactly.
...
#### 5.5.1B How the stored prior scale vector $s_0^{vdW}$ is reconstructed from van der Werf Table 2 confidence intervals

To make the frozen external-centering artifact auditable, this companion documents the reconstruction rule that reproduces the currently stored $s_0^{vdW}$ exactly from the published age- and sex-specific **adjusted IRR** confidence intervals in Table 2 of van der Werf et al. This is a documentation rule for the already frozen artifact, not a new tuning step during posterior fitting.

For any adjusted IRR reported with 95% confidence interval $(L, U)$, recover the log-scale standard error by the usual Wald relation:

$$
SE\{\log(IRR)\} = \frac{\log(U) - \log(L)}{2 \times 1.96} = \frac{\log(U) - \log(L)}{3.92}.
$$

Table 2 reports adjusted IRRs from negative binomial regression and the paper states that robust standard errors were used in the modeling stage. The published confidence intervals therefore provide the recoverable uncertainty information used here for the frozen prior-width artifact.

The current incidence design is

$$
X_i^{inc} = \left( z_i, x_{20,i}, x_{30,i}, z_i x_{20,i}, z_i x_{30,i} \right)^{T},
$$

with reference cell Female, age $<20$ years. The current frozen prior means $\mu^{vdW}$ come from the median rates through the rate $\rightarrow$ one-year risk $\rightarrow$ logit conversion described in Section 5.5.1A. By contrast, the current frozen prior scales $s_0^{vdW}$ are calibrated from the widths of the adjusted IRR confidence intervals in Table 2.

A crucial asymmetry must be stated explicitly: the external table is used **rate-wise for the prior center** and **CI-width-wise for the prior scale**. The CI-based quantities are therefore being used as transparent literature-derived prior-width calibrators, not as an exact same-scale sampling covariance matrix for the logit-risk contrasts.

The coefficient mapping for the current frozen artifact is:

- $\beta_1$ for $z_i$: male-versus-female contrast in the reference age band ($<20$). Table 2 prints the Female-to-male IRR for this row; reversing the comparison changes the sign of $\log(IRR)$ but not its standard error, so the same recovered SE is used as the prior SD.
- $\beta_2$ for $x_{20,i}$: female age 20–29 versus female $<20$.
- $\beta_3$ for $x_{30,i}$: female age 30–39 versus female $<20$. In the current frozen artifact, the slot $x_{30,i}=I(\text{age}_i \ge 30)$ is anchored to the external **30–39** row; it is not based on a pooled $\ge 30$ summary.
- $\beta_4$ for $z_i x_{20,i}$: male-versus-female age-pattern difference for 20–29. Because the publication does not provide covariance terms across the sex-specific age-effect estimates, the current frozen artifact uses the zero-covariance rule
  $$
  s_{0,4} = \sqrt{SE_{M,20\text{–}29}^{2} + SE_{F,20\text{–}29}^{2}}.
  $$
- $\beta_5$ for $z_i x_{30,i}$: male-versus-female age-pattern difference for 30–39, using
  $$
  s_{0,5} = \sqrt{SE_{M,30\text{–}39}^{2} + SE_{F,30\text{–}39}^{2}}.
  $$

The exact reconstructions are:

| Coefficient in current design | External Table 2 quantity used for scale | Reconstruction | Resulting frozen SD |
|---|---|---:|---:|
| $\beta_1$ for $z_i$ | Female-to-male adjusted IRR at age $<20$: $0.53\ (0.41, 0.69)$ | $[\log(0.69)-\log(0.41)]/3.92$ | $0.132789397422$ |
| $\beta_2$ for $x_{20,i}$ | Women 20–29 adjusted IRR versus women $<20$: $2.34\ (1.66, 3.28)$ | $[\log(3.28)-\log(1.66)]/3.92$ | $0.173731076538$ |
| $\beta_3$ for $x_{30,i}$ | Women 30–39 adjusted IRR versus women $<20$: $2.25\ (1.55, 3.28)$ | $[\log(3.28)-\log(1.55)]/3.92$ | $0.191221553945$ |
| $\beta_4$ for $z_i x_{20,i}$ | Men 20–29 adjusted IRR $2.61\ (1.74, 3.92)$ and Women 20–29 adjusted IRR $2.34\ (1.66, 3.28)$ | $\sqrt{([\log(3.92)-\log(1.74)]/3.92)^2 + ([\log(3.28)-\log(1.66)]/3.92)^2}$ | $0.270393197518$ |
| $\beta_5$ for $z_i x_{30,i}$ | Men 30–39 adjusted IRR $1.49\ (0.94, 2.36)$ and Women 30–39 adjusted IRR $2.25\ (1.55, 3.28)$ | $\sqrt{([\log(2.36)-\log(0.94)]/3.92)^2 + ([\log(3.28)-\log(1.55)]/3.92)^2}$ | $0.302838606651$ |

These five reconstructions reproduce the currently stored vector exactly:

$$
s_{0}^{vdW} = \left(0.132789397422, 0.173731076538, 0.191221553945, 0.270393197518, 0.302838606651\right)^{T}.
$$

No pooled $\ge 30$ CI and no full $5 \times 5$ covariance matrix are reconstructed from the publication. Until a more directly aligned external source provides that information, the values above remain the locked, transparent, reproducible prior-width artifact for the current Stage 8 companion.
...
Current prior scale vector (exactly reproduced by the reconstruction rule in Section 5.5.1B):
...
### 9.2A Distinct roles of rate information and confidence-interval information

The current van der Werf prior artifact uses the external synthesis in two different ways and this distinction should be stated explicitly:

1. the **median incidence rates** determine the prior center after rate $\rightarrow$ one-year risk $\rightarrow$ logit conversion and representation on the current incidence design matrix;
2. the **adjusted IRR confidence-interval widths** determine the frozen prior-scale vector $s_0^{vdW}$ via recovered $SE\{\log(IRR)\}$ values, with a zero-covariance combination rule for the interaction terms because Table 2 does not publish cross-estimate covariance terms.

Accordingly, $\mu^{vdW}$ and $s_0^{vdW}$ come from the same external paper but not from the same numerical object inside that paper. The current $x_{30}=I(\text{age}\ge30)$ slot is anchored to the published 30–39 row, and the stored $s_0^{vdW}$ vector is a transparent prior-width calibration artifact rather than a reconstructed full covariance matrix for the logit-risk contrasts.
