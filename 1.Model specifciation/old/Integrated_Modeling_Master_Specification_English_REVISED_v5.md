...
8. **Retain both separate-cohort and merged views in the primary structural analysis set**
   ...
   - For Stage 7 and Stage 8 cure modeling, the merged `site_adjusted` view must be further decomposed into explicit site-placement structures so that site terms can be compared in incidence, latency, or both.
...
Operationally, the merged Stage 7 formula plan must treat `site_adjusted` as a **coarse cross-stage branch label** rather than as a single fixed formula.

Accordingly, the required merged Stage 7 structural comparison set is:

- site-free base
- site-free age-by-sex interaction
- site-adjusted with `site` in latency only
- site-adjusted with `site` in incidence only
- site-adjusted with `site` in both incidence and latency

This decomposition is required because the merged analysis is intended to ask whether PNU/SNU difference is more compatible with:
...
When a semiparametric Cox-latency cure fit and a no-cure Cox PH fit both expose comparable likelihood metrics, an additional Cox-based same-family contrast may also be exported as a supportive extension.

However, the required minimum block remains the four fully parametric families above. Failure of the optional semiparametric pair must not invalidate the required parametric family-matched contrast export.
...
- `reference_chisq_proxy_nonregular_caution`
...
The reference-chi-square proxy label is permitted only as an implementation-facing supportive placeholder when a true nonregular bootstrap calibration has not been run. It must remain explicitly marked as nonregularity-cautioned and must not be interpreted as a decisive textbook LRT.
...
- explicit merged site-placement labels:
  ...
  - `site_in_latency_only`
  - `site_in_both`
- implementation-level merged formula variants that preserve the coarse branch and the finer site-placement decomposition:
  - `base_sitefree`
  - `interaction_sitefree`
  - `base_siteadjusted` = site in latency only
  - `interaction_siteadjusted` = site in latency only
  - `base_siteadjusted_incidenceonly`
  - `interaction_siteadjusted_incidenceonly`
  - `base_siteadjusted_both`
  - `interaction_siteadjusted_both`
...
### 10.4A Merged site-placement mapping for Stage 7

For cross-stage compatibility, Stage 6 and other upstream backbone exports may continue to use the coarse merged variants:

- `merged__site_free`
- `merged__site_adjusted`

However, within Stage 7 frequentist cure modeling, `merged__site_adjusted` must fan out into explicit site-placement structures:

- `site_in_latency_only`
- `site_in_incidence_only`
- `site_in_both`

while `merged__site_free` corresponds to:

- `site_in_neither`

Therefore:

- `site_branch` remains the coarse join field used across stages
- `site_placement_label` is the required within-Stage-7 decomposition field for interpretation
- any Stage 7 write-up about merged site effects must use `site_placement_label`, not only the coarse `site_adjusted` branch name
